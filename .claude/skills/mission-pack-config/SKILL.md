...
| Smart AI Pass (AI squad behaviour: contact/flanking/fire control/morale, reinforcement, artillery, airborne, garrison/clear orders, survivor regroup, LAMBS split) | `references/smart-ai-pass.md` | Off by default; server/HC only; per-behaviour switches; other WMP features' AI auto-excluded |
...
| Vehicle Appearance (recolor, show/hide model parts) | `references/vehicle-appearance.md` | No `MissionConfig` file — call/ZEN only |
