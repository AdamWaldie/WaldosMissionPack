...
jumpers, auto-drop-on-approach and map markers. The ZEN module's Airframe dropdown is populated from
`Waldo_Paradrop_AircraftClasses` when a mission maker has set that allowlist, otherwise it falls back
to `Waldo_fnc_ResolveVehicleClassPool`, discovering every public cargo `Air` class live in the
running modset (vanilla or third-party) instead of showing an empty list. Use the ZEN module or `ParadropCreateDropZone` for a
