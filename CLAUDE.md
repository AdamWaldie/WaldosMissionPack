...
**The release tag is the single source of truth for the shipped version — no manual `description.ext` bump is required before tagging.** `releaseVerificationAndDeployment/set_description_version.py` rewrites just the version digits inside `onLoadName` (preserving the rest of the field) and is used from two places:

- `deploy.sh` rewrites the version in its own build checkout before packing (a leading `v` and any prerelease suffix like `-rc1` are stripped), then calls `generateLoadingScreen.py` with the tag so the packaged mission's title and cover always match the tag being released. This edit is transient — local to that build's working copy, never committed.
- `.github/workflows/deploy.yml`'s `sync-main-version` job runs after a successful build, checks out the current tip of `main` separately, applies the same version bump and regenerates `Pictures/loading.jpg`, and pushes a `[skip ci]` commit if either file changed — so the repository's own visible state (README, GitHub file view) reflects the release automatically.

`.github/workflows/update-cover.yml` remains as a safety net: it regenerates and commits `Pictures/loading.jpg` whenever `description.ext` changes on `main` for any other reason (e.g. a manual dev-time version bump outside a release).
