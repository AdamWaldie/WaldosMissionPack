...
- Zeus END-key kill restore - a later Arma engine update made the vanilla Zeus "press END to instantly kill" shortcut respect `allowDamage`, so any damage-immune unit (including WMP's own SafeStart/ENDEX freeze) silently stopped dying to it. WMP reinstalls the same hover-and-press-END behaviour (or kills every box-selected entity at once) through its own `Waldo_fnc_KillUnit`, which forces `allowDamage true` first and is ACE-medical-aware, so END works again regardless of what disabled damage.
