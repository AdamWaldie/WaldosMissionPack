...
- Smart AI Pass - optional behaviour improvements for non-player AI squads:
  - a contact and post-contact state ladder;
  - flanking in covered bounds with street crossing;
  - fire control, morale, retreat and surrender;
  - anti-armour and vehicle drills, and grenade evasion;
  - contact reports, reinforcement, artillery and counter-battery;
  - airborne reinforcement, garrison and clear-building orders, and survivor regroup.

  It runs only on the server and headless clients inside a fixed per-tick budget, with a switch per behaviour, LAMBS split-by-feature support, and automatic exclusion of player groups and other WMP features' AI. Controlled from `MissionConfig\aiConfig.sqf` and the **AI Control** and **AI Orders** Zeus modules.
