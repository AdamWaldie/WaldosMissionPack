...
Both instruction sets are always available - a squad leader elsewhere can command a transport remotely through the controls above whenever it is AI-driven, and anyone already aboard (any seat, not just the requester or a leader) can instead fly or drive it themselves. The driver seat itself is never locked, so a player can also just walk up and board it directly like any other vehicle whenever it is empty - **Take Manual Control** exists for the more common case, where the seat is already occupied by the AI pilot:
...
The driver seat is left open — a player may board it directly like any other vehicle, with no forced detour through Take Manual Control. Registration disables fleeing/panic on the captured AI service crew instead (`allowFleeing 0`), so it stays in the vehicle and the seat stays occupied under normal operation — a driver who bails out under fire otherwise stranded the vehicle the same way a dead driver would, just without the monitor's driver-death check ever catching it. A passenger who boards later as cargo is unaffected. A registered vehicle that becomes too heavily damaged to remain effective — `Waldo_Transport_MaxEffectiveDamage` in `MissionConfig\logisticsConfig.sqf`, default `0.8` — is written off the service pool the same way an outright loss is, and (unlike a self-evident loss) every player on the service's `allowedSides` gets a warning card naming it, since a vehicle quietly vanishing from availability otherwise has no explanation.
