...
- this client's own intro-sequence timing (`mission-flow`/`infotext-timing`): real measured seconds for the playable-client readiness wait, WMP's short setup cover, when control was available, and how much longer the cosmetic title kept typing. The client state is recorded for diagnosis but is not used as a readiness gate. `ERROR` only means the playable client never reached the live display/player/engine-ready conditions within 60 seconds. The intro does not wait for `WALDO_INIT_COMPLETE` or any unrelated feature. See [Mission Intro and Title Text](Mission-Intro-Or-Title-Text) for the sequence.
