...
- this client's own intro-sequence timing (`mission-flow`/`infotext-timing`): real measured seconds for each stage of `Waldo_fnc_InfoText` - the wait for the display to exist, the per-client streaming-settle wait, the fake loading screen and its fades, the wait on `WALDO_INIT_COMPLETE`, total time until control returned, and how much longer the text kept typing after that. `ERROR` only when the `WALDO_INIT_COMPLETE` wait hit its 60-second cap; every other result is a measurement, not a judgment, so a mission maker reporting a slow or mistimed intro can point at the exact stage instead of guessing. See [Mission Intro and Title Text](Mission-Intro-Or-Title-Text) for what each stage does.
