...
| [Obituary and Confirmed-Death Reporting](Optional-Feature-Systems#obituary-and-confirmed-death-reporting) | "Pronounce Dead" ACE self-interaction (vanilla Medic trait or ACE Medic/Doctor role) listing nearby corpses individually, with a formatted KIA report; feeds the AAR's "Confirmed deaths" section. Enabled by default | `MissionConfig\interfaceConfig.sqf`; runs automatically from `initPlayerLocal.sqf`; intentionally no ZEN module |
...
| [Helicopter, Ground and Boat Transport](Transport-Services) | Reusable server-reserved AI transport pools with named management, physical pickup, destination, disembark and RTB lifecycles | Register AI-crewed vehicles by script or the focused WMP Logistics ZEN modules |
