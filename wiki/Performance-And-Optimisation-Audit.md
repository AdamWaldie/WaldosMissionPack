...
| 31 August 2026 | Economy's resource-zone bridge now clears obsolete pre-V4 player action flags once at startup instead of walking every player every two seconds. | Those compatibility flags exist only on player objects that survived an older bridge version in the same running mission. Arma creates a new player object for a joining player, so JIP cannot inherit those stale object variables. | The same current-player compatibility cleanup still runs. The existing two-second dynamic-zone anchor/action reconciliation, object-bound JIP actions, capture requests and information actions are unchanged. | Static pre-loop cleanup/no-loop-player-scan assertion plus an in-engine current-player, JIP, dynamic-zone create/delete and purge/re-enable check. |
| 31 August 2026 | The electronic-warfare tracker prune worker now stops after the tracker registry becomes empty and restarts lazily when a later tracker is planted. | Arma removes deleted targets, while WMP must remove their custom side-filtered tracker metadata and publish that removal for current/JIP clients. No custom metadata remains to prune after the final entry is gone. | Active trackers retain the same five-second validity check, side visibility, local marker rendering and global/JIP registry update. Later tracker creation uses the existing guarded startup path. | Static active-registry/empty-exit/restart-guard assertions plus current-client/JIP target death, deletion, empty idle and later-recreate checks. |
| 31 August 2026 | The signal-tracker client renderer now exists only while its registry is non-empty. | Arma transports the tracker registry but cannot draw side-filtered local markers. One guarded registry listener wakes WMP presentation on remote changes; the listen server receives an explicit local wake-up because publication is not replayed to the publishing machine. | Initial/JIP trackers, side filtering, two-second live position/direction updates, final marker cleanup and later tracker recreation retain their existing behaviour. Empty missions and post-removal clients run no tracker redraw worker. | Static listener/empty-exit/listen-host/restart assertions plus dedicated initial, JIP, listen-host, target movement, remove-last and recreate checks. |
...
|---|---:|---:|---|
| Scanner findings | 99 at branch checkpoint | 94 | Five further recurring-pattern findings removed after the checkpoint |
| High-severity findings | 11 at branch checkpoint | 10 | One high-severity pattern removed; remaining findings require human review |
| Recurring world-scan findings | 8 at direct-request checkpoint | 7 | The two-second Economy player scan was removed |
| Loops without a nearby recognised single-start guard | 66 | 66 | No guard finding was hidden or baseline-suppressed; remaining medium findings require human review |
