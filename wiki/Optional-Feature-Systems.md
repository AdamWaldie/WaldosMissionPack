...
These mission-maker systems are repeat-safe, configured outside their implementation files, and provide explicit stop or cleanup functions. Shared settings live in `MissionConfig\SharedFeatureDefaults.sqf`; interface-only settings live in `MissionConfig\PlayerLocalFeatureDefaults.sqf`; server-only limits and asset pools live in `MissionConfig\ServerFeatureDefaults.sqf`. The init files retain activation and locality handling. Most systems are disabled by default. The accessibility aid is enabled only for its original allowlisted recipient.
...
Set `Waldo_Hazard_Enable = true`, then register any number of zones. A zone accepts a trigger, marker name, moving object emitter, `[position, radius]`, or `[position, axisA, axisB, angle, rectangle]`, plus an extensible profile hash map. Pre-planned zones should be registered consistently on clients (for example from shared mission initialisation); the ZEN/runtime path performs authoritative live registration, ordered activation and JIP replay automatically.
