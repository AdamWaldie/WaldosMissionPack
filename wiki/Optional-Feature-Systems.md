...
Call `Waldo_fnc_TreatmentFeedbackInit` or `Waldo_fnc_TreatmentFeedbackStop` on interface clients after changing the player-local settings. This feature intentionally has no ZEN module. Success notifications listen for both `ace_treatmentSucceded` (ACE's current typo'd event identifier) and `ace_treatmentSucceeded` (the corrected spelling), so the feature keeps working unchanged whichever one a given mission's ACE build actually fires.
...
Eligible players can rapidly enable or disable it through **ACE Self Interact > WMP Options > WMP HUD**. The same submenu opens a custom presentation screen for permitted content, scale and opacity; those choices cannot override mission eligibility or information restrictions. Cross-interface colour vision and reduced motion live under **WMP Options > Accessibility Settings**. Blue vanilla addActions provide the fallback. WMP HUD intentionally has no ZEN module. See the [complete WMP HUD guide](WMP-HUD).
