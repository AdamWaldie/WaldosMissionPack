...
`Waldo_Hazard_NotifyTransitions` enables player-local WMP notification cards on zone entry and exit and defaults to `true`; `Waldo_Hazard_NotificationDuration` defaults to six seconds. The separate continuous exposure card defaults off (`Waldo_Hazard_ShowStatus = false`) so one zone does not produce two simultaneous UI panels. A profile may opt back in with `showStatus = true`. Each profile can also override `notifyTransitions`, `notificationDuration`, `enterMessage`, `exitMessage`, `enterState` and `exitState`. Notifications use a per-zone channel, so a boundary transition replaces its own stale card without suppressing simultaneous warnings from other zones.
