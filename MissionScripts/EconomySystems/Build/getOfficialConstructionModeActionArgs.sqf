 ...
 */
        ...
        [
            "Construction Mode",
            {
                ...
                if (isNull _parent) exitWith {diag_log "[WMP ECONOMY UI] Construction display unavailable: main display missing";};
                ...
                private _consumesSource = !isNull _target && {_target getVariable ["WaldoEcoBuild_IsConstructionVehicle", false]};
                _disp setVariable ["WaldoEcoBuild_ConsumesConstructionSource", _consumesSource];
                ...
                _button ctrlSetPosition [0.69, 1.03, 0.23, 0.055];
                _button ctrlSetText (if (_consumesSource) then {"Deploy + Consume"} else {"Place"});
                _button ctrlCommit 0;

                private _consumeWarning = _disp ctrlCreate ["RscText", -1];
                _consumeWarning ctrlSetPosition [0.43, 0.955, 0.66, 0.055];
                _consumeWarning ctrlSetText "ONE USE: SOURCE IS CONSUMED";
                _consumeWarning ctrlSetTextColor [1, 0.82, 0.35, 1];
                _consumeWarning ctrlSetBackgroundColor [0.28, 0.12, 0.02, 0.82];
                _consumeWarning ctrlShow _consumesSource;
                _consumeWarning ctrlCommit 0;
                ...
                private _refresh = {
                    ...
                    private _validClass = _className != "" && {isClass (configFile >> "CfgVehicles" >> _className)};
                    if (!_validClass) then {
                        _effectRows pushBack [format ["CONFIG ERROR: missing class %1", _className], [1, 0.35, 0.35, 1]];
                        [_display getVariable ["WaldoEcoBuild_PlayerEffectLines", []], _effectRows] call _setLines;
                    };

                    private _consumesSource = _display getVariable ["WaldoEcoBuild_ConsumesConstructionSource", false];
                    _button ctrlSetText (if (_consumesSource) then {"Deploy + Consume"} else {"Place"});
                    _button ctrlEnable (_status == "build" && {_validClass});
                ...
                private _startPlacement = {
                    ...
                    if (_className == "" || {!(isClass (configFile >> "CfgVehicles" >> _className))}) exitWith {
                        ...
                        diag_log format ["[WMP ECO][BUILD][ERROR] public placement rejected name=%1 reason=INVALID_CFGVEHICLES_CLASS configuredClass=%2", _buildName, _className];
                    };
                    ...
                    private _consumesSource = _display getVariable ["WaldoEcoBuild_ConsumesConstructionSource", false];
                    private _sourceText = if (_consumesSource) then {
                        "Confirming converts this construction vehicle into the site."
                    } else {
                        "This construction base remains available after confirmation."
                    };
                    [format ["PLACEMENT ACTIVE // Aim at the site and rotate as needed. %1", _sourceText], 12] call Waldo_fnc_EcoCore_notifyActorLocal;
                };
                ...
                _button ctrlAddEventHandler ["ButtonClick", {
                    ...
                    [_display] call Waldo_fnc_EcoCore_closePromptDisplayIfDedicated;
                }];
