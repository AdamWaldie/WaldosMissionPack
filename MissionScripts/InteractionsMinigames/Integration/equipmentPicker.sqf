...
[_head, "<t size='%1' color='#EEE9D8'>FIELD EQUIPMENT</t><br/><t color='#A5A697'>Select a standalone local operating procedure. Party-table state is unaffected.</t>", 1.20, 0.62] call Waldo_fnc_MiniGameEquipmentFitStructuredText;
...
_display setVariable ["Waldo_IMG_PickerRefresh", {
    ...
    private _detailsTemplate = "<t size='%1' color='#F2BE55'>"
        + (_profile getOrDefault ["title", "EQUIPMENT"])
        + "</t><br/><t color='#9C9D8F'>"
        + (_profile getOrDefault ["manufacturer", ""])
        + "<br/>"
        + (_profile getOrDefault ["model", ""])
        + "</t><br/><br/><t color='#EEE9D8'>"
        + (_profile getOrDefault ["objective", ""])
        + "</t><br/><br/><t color='#D9B85C'>DIFFICULTY: "
        + _difficulty
        + "</t><br/><t color='#C9C6B8'>Local solo procedure<br/>Result does not change party-game votes or readiness.</t>";
