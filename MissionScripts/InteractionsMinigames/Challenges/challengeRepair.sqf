...
_legend ctrlSetStructuredText parseText format ["<t align='center' color='#EEE9D8'>CALIBRATION LIMIT</t><br/><t align='center' color='#7FD59B'>[OK] TARGET +/-%1 Nm</t><br/><t align='center' color='#F2BE55'>YELLOW = WRENCH</t><br/><t align='center' color='#7FD59B'>GREEN = TARGET</t>", _tolerance];
...
_display setVariable ["Waldo_MG_RP_SelectBolt", {
    ...
    if (!isNull _readout) then {_readout ctrlSetText format ["BOLT %1 TARGET // %2 Nm", _index + 1, _target];};
    {
...
_display setVariable ["Waldo_MG_RP_Apply", {
    ...
    _display setVariable ["Waldo_MG_RP_Selected", -1];
    private _readout = _display getVariable ["Waldo_MG_RP_TargetReadout", controlNull];
    if (!isNull _readout) then {_readout ctrlSetText format ["BOLT %1 // [OK] TORQUED", _index + 1];};
