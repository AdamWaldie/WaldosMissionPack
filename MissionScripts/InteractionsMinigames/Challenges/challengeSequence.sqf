...
for "_index" from 0 to (_padCount - 1) do {
    ...
    // Cover the coloured guard as well as the button. Leaving the guard exposed
    // made otherwise identical playback cues appear to glow with different
    // colours and brightness.
    private _cue = [_display, "RscText", [_x - 0.35, _y - 0.35, _buttonW + 0.7, _buttonH + 0.7], format ["playback cue %1 %2", _index + 1, _identity select 0]] call Waldo_fnc_MiniGameEquipmentCreateControl;
...
[_attention, "<t align='center' size='%1' color='#F2C247'>EYES ON CONTROL PANEL</t><br/><t align='center' color='#EEE9D8'>PLAYBACK STARTING</t>", 1.35, 0.72] call Waldo_fnc_MiniGameEquipmentFitStructuredText;
...
_display setVariable ["Waldo_MG_SQ_Activate", {
    ...
    private _inputReadout = _display getVariable ["Waldo_MG_SQ_InputReadout", controlNull];
    if (!isNull _inputReadout) then {
        _inputReadout ctrlSetText format ["ENTERED: %1", _entered joinString "  >  "];
        [_inputReadout, 0.034, 0.016] call Waldo_fnc_MiniGameEquipmentFitText;
    };
    private _stage = _display getVariable ["Waldo_MG_SQ_Stage", controlNull];
