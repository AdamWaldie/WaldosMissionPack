...
private _load = _target addAction [
    ...
    [], 1.4, true, true, "", "_this distance _target < 6 && {alive _target} && {abs speed _target < 1} && {(count (getVehicleCargo _target)) + (count (_target getVariable ['Waldo_Recovery_VirtualPackages', []])) < (_target getVariable ['Waldo_Recovery_CarrierCapacity', 1])}", 6
];
private _unload = _target addAction [
    ...
    [], 1.4, true, true, "", "_this distance _target < 6 && {alive _target} && {abs speed _target < 1} && {(count (getVehicleCargo _target)) + (count (_target getVariable ['Waldo_Recovery_VirtualPackages', []])) > 0}", 6
];
