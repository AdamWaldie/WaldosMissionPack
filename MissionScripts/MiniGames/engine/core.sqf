...
Waldo_MG_fnc_refreshTableConsensusServer = {
    ...
    _table setVariable ["Waldo_MG_TableSelectedGame", _selectedGame, true];
    _table setVariable ["Waldo_MG_TableRequiredVotes", _requiredVotes, true];
    _table setVariable ["Waldo_MG_TablePhase", _phase, true];
    _table setVariable [
...
Waldo_MG_fnc_releaseUnitSeatServer = {
    ...
    if (!isNull _table) then {
        [_table, _unit, _seatIndex] call Waldo_MG_fnc_battleshipHandleDepartureServer;
        [_table, _unit, _seatIndex] call Waldo_MG_fnc_whosWhoHandleDepartureServer;
        [_table, _unit, _seatIndex] call Waldo_MG_fnc_shotgunHandleDepartureServer;
        [_table, _unit, _seatIndex] call Waldo_MG_fnc_checkersFinishForfeitServer;
        [_table, _unit, _seatIndex] call Waldo_MG_fnc_rpsFinishForfeitServer;
...
Waldo_MG_fnc_reconcileOneTableServer = {
    ...
    if (!isServer || {isNull _table}) exitWith {};
    [_table] call Waldo_MG_fnc_enforceInvulnerableLocal;
    [_table] call Waldo_MG_fnc_battleshipReconcilePlayersServer;
    [_table] call Waldo_MG_fnc_whosWhoReconcilePlayersServer;
    [_table] call Waldo_MG_fnc_shotgunReconcilePlayersServer;
    [_table] call Waldo_MG_fnc_shotgunProgressServer;
    [_table] call Waldo_MG_fnc_checkersReconcilePlayersServer;
...
Waldo_MG_fnc_submitJoinRequestLocal = {
    ...
    if (isNull _table) exitWith {
        hintSilent "That party table is unavailable.";
    };
    if (!alive player) exitWith {
        hintSilent "You must be alive to sit at the table.";
    };
    ...
    hintSilent "Requesting a seat from the table host...";
};
...
Waldo_MG_fnc_submitLeaveRequestLocal = {
    ...
    hintSilent "Leaving the party table...";
};
...
Waldo_MG_fnc_submitVoteRequestLocal = {
    ...
    if (isNull _table || {_gameId == ""}) exitWith {
        hintSilent "Select an available game before voting.";
    };
    ...
    hintSilent "Submitting your table vote...";
};
...
Waldo_MG_fnc_submitReadyRequestLocal = {
    ...
    hintSilent "Updating your ready state...";
};
...
Waldo_MG_fnc_exitSpectatorLocal = {
    ...
    if (!_silent && {!isNull _table}) then {
        hintSilent "You stopped spectating the table game.";
    };
};
...
Waldo_MG_fnc_openSpectatorLocal = {
    ...
    if (!alive player || {(lifeState player) == "INCAPACITATED"} || {(vehicle player) != player}) exitWith {
        hintSilent "You must be alive and on foot to spectate a table game.";
    };
    if (!isNull (player getVariable ["Waldo_MG_SeatedTable", objNull])) exitWith {
        hintSilent "Leave your current table seat before spectating another game.";
    };
    if (!([_table] call Waldo_MG_fnc_isTableGameActive)) exitWith {
        hintSilent "That table does not currently have a game to spectate.";
    };
    ...
    if (_occupants >= 1) then {
        if (_consensusGame == "") then {
            ...
            if (_phase == "READY") then {
                ...
                _statusLineTwo = if (_consensusGame == "checkers") then {
                    "Launching the authoritative Checkers board..."
                } else {
                    if (_consensusGame == "rps") then {
                        "Preparing the hidden-choice reveal..."
                    } else {
                        if (_consensusGame == "blackjack") then {
                            "Opening the authoritative Blackjack table..."
                        } else {
                            if (_consensusGame == "chess") then {
                                "Launching the authoritative Chess board..."
                            } else {
                                if (_consensusGame == "poker") then {
                                    "Shuffling the authoritative Poker deck..."
                                } else {
                                    if (_consensusGame == "uno") then {
                                        "Dealing the authoritative UNO hands..."
                                    } else {
                                        if (_consensusGame == "battleship") then {
                                            "Opening two private fleet deployment grids..."
                                        } else {
                                            "The game module is preparing."
                                        }
                                    }
                                }
                            }
...
Waldo_MG_fnc_openLobbyLocal = {
    ...
    if (isNull _table || {(player getVariable ["Waldo_MG_SeatedTable", objNull]) != _table}) exitWith {
        hintSilent "Sit at the party table before opening its lobby.";
    };
    private _parent = findDisplay 46;
    if (isNull _parent) exitWith {
        hintSilent "Mini Games lobby display is unavailable.";
    };
