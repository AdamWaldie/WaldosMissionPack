...
Waldo_MG_fnc_submitPokerActionRequestLocal = {
    ...
    if ((count _pending) >= 2 && {(diag_tickTime - (_pending param [1, -10])) < 1.5}) exitWith {
        hintSilent "Waiting for the table host to answer your previous Poker action...";
    };
...
Waldo_MG_fnc_openPokerLocal = {
    ...
    ) exitWith {hintSilent "No active Poker match is available to this viewer.";};
    private _parent = findDisplay 46;
    if (isNull _parent) exitWith {hintSilent "The Poker display is unavailable.";};
    {
