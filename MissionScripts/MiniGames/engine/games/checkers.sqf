...
Waldo_MG_fnc_submitCheckersResetRequestLocal = {
    ...
    hintSilent "Returning the finished Checkers match to the lobby...";
};
...
Waldo_MG_fnc_handleCheckersSquareClickLocal = {
    ...
    if (([_table] call Waldo_MG_fnc_getTableActiveGameId) != "checkers") exitWith {
        hintSilent "That Checkers match is no longer active.";
    };
    if ((_table getVariable ["Waldo_MG_CheckersWinner", 0]) != 0) exitWith {
    ...
    if (_pieceSide == _side) exitWith {
        if (_forcedFrom >= 0 && {_index != _forcedFrom}) then {
            hintSilent "Continue the capture chain with the highlighted piece.";
        } else {
            ...
            if ((count _moves) <= 0) then {
                if ([_board, _side] call Waldo_MG_fnc_checkersSideHasCapture) then {
                    hintSilent "Another piece has a compulsory capture.";
                } else {
                    hintSilent "That piece has no legal move.";
                };
            } else {
    ...
    if (!_isLegal) exitWith {
        hintSilent "Choose one of the highlighted destination squares.";
    };
...
Waldo_MG_fnc_openCheckersLocal = {
    ...
    if (
        ...
        hintSilent "No active Checkers match is available to this viewer.";
    };
    private _parent = findDisplay 46;
    if (isNull _parent) exitWith {
        hintSilent "The Checkers display is unavailable.";
    };
