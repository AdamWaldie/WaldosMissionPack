...
Waldo_MG_fnc_submitUNOActionRequestLocal = {
    ...
    if ((count _pending) >= 2 && {(diag_tickTime - (_pending param [1, -10])) < 1.5}) exitWith {
        hintSilent "Waiting for the table host to answer your previous UNO action...";
        false
    };
...
Waldo_MG_fnc_handleUNOCardClickLocal = {
    ...
    if ((_state param [0, "IDLE"]) != "PLAYING" || {(_state param [2, -1]) != _role}) exitWith {
        hintSilent "Wait for your UNO turn.";
    };
    ...
    if (
        ...
        hintSilent "Your private hand is synchronizing. Try again in a moment.";
    };
    ...
    if ((_state param [11, -1]) == _role && {(_privatePayload param [3, -1]) != _cardId}) exitWith {
        hintSilent "After drawing, only the newly drawn card may be played.";
    };
    if (!([_card, _hand, _state param [4, []], _state param [5, -1], _state param [9, -1]] call Waldo_MG_fnc_unoCanPlayFromHand)) exitWith {
        hintSilent "That card is not legal against the active colour, symbol, or draw chain.";
    };
    ...
    if (_existing >= 0) then {
        ...
        if (!_compatible) exitWith {hintSilent "Only exact duplicate numbered cards can be selected together.";};
        if ((count _selected) >= 2) exitWith {
            hintSilent "This ruleset batches at most the two duplicate copies.";
        };
...
Waldo_MG_fnc_submitUNOButtonLocal = {
    ...
    if (_action == "PLAY") exitWith {
        private _selected = +(_display getVariable ["Waldo_MG_UNOSelectedCardIds", []]);
        if ((count _selected) <= 0) exitWith {hintSilent "Select a playable UNO card first.";};
        ...
        if ((count _firstCard) <= 0) exitWith {hintSilent "That card selection is stale.";};
...
Waldo_MG_fnc_openUNOLocal = {
    ...
    ) exitWith {hintSilent "No active UNO match is available to this viewer.";};
    private _parent = findDisplay 46;
    if (isNull _parent) exitWith {hintSilent "The UNO display is unavailable.";};
    {
