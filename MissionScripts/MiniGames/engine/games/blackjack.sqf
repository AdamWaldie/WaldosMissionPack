...
Waldo_MG_fnc_submitBlackjackActionRequestLocal = {
    ...
    if ((count _pending) >= 2 && {(diag_tickTime - (_pending param [1, -10])) < 1.5}) exitWith {
        hintSilent "Waiting for the table host to answer your previous Blackjack action...";
        false
    };
...
Waldo_MG_fnc_openBlackjackLocal = {
    ...
    if (
        ...
        hintSilent "No active Blackjack table is available to this viewer.";
    };
    private _parent = findDisplay 46;
    if (isNull _parent) exitWith {
        hintSilent "The Blackjack display is unavailable.";
    };
    {
