...
Waldo_MG_fnc_openConnectFourLocal = {
    ...
    if (!hasInterface || {isNull player} || {isNull _table}) exitWith {};
    if (!([_table, _spectating] call Waldo_MG_fnc_isValidGameViewerLocal) || {([_table] call Waldo_MG_fnc_getTableActiveGameId) != "connectfour"}) exitWith {hintSilent "No active Connect Four match is available.";};
