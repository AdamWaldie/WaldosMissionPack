...
Waldo_MG_fnc_openLiarsDiceLocal = {
    ...
    if (!hasInterface || {isNull _table} || {!([_table,_spectating] call Waldo_MG_fnc_isValidGameViewerLocal)} || {([_table] call Waldo_MG_fnc_getTableActiveGameId)!="liarsdice"}) exitWith {hintSilent "No active Liar's Dice match is available.";};
