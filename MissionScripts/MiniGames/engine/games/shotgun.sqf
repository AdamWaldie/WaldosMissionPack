...
Waldo_MG_fnc_submitShotgunActionRequestLocal = {
    ...
    if ((count _pending) >= 2 && {(diag_tickTime - (_pending param [1, -10])) < 1.5}) exitWith {
        ["Waiting for the table host to answer your previous Shotgun Roulette action..."] call Waldo_MG_fnc_notifyLocal;
        false
    };
    ...
    ["SHOTGUN", _table, _token, _request param [3, -1], _request] call Waldo_MG_fnc_submitRequestLocal;
    true
};
...
Waldo_MG_fnc_openShotgunLocal = {
    ...
    if (
        ...
        ["No active Shotgun Roulette table is available to this viewer."] call Waldo_MG_fnc_notifyLocal;
    };
    private _parent = findDisplay 46;
    if (isNull _parent) exitWith {["The Shotgun Roulette display is unavailable."] call Waldo_MG_fnc_notifyLocal;};
    {
