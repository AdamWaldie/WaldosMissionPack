...
Waldo_MG_fnc_submitRPSActionRequestLocal = {
    ...
    if ((count _pending) >= 2 && {(diag_tickTime - (_pending param [1, -10])) < 1.5}) exitWith {
        hintSilent "Waiting for the table host to answer your previous choice...";
        false
    };
...
Waldo_MG_fnc_openRPSLocal = {
    ...
    if (
        ...
        hintSilent "No active Rock Paper Scissors match is available to this viewer.";
    };
    private _parent = findDisplay 46;
    if (isNull _parent) exitWith {
        hintSilent "The Rock Paper Scissors display is unavailable.";
    };
    {
