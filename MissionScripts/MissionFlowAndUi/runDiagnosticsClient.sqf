...
if (_acreEnabled) then {
    ...
    if !(_edenRadioSetup isEqualTo "") then {
        ["radio", "acre-eden-radio-attribute", "ERROR", format ["This unit has an Eden ACRE Radio Setup attribute (%1). It can overwrite acreConfig.sqf during startup. Clear that unit attribute and let WMP own the initial assignment. If it keeps winning on this mission specifically, ACRE is likely taking longer to initialize than Waldo_ACRE2_Config's readinessTimeoutSeconds (MissionConfig\\acreConfig.sqf, default 120s) - WMP's own overwrite still runs automatically once ACRE catches up (see [WMP ACRE] RPT entries for the retry), but raising this value avoids the visible delay on a heavy modset.", _edenRadioSetup]] call _add;
    } else {
