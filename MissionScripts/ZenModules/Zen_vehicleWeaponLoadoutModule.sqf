...
// "Copy Weapon From" collects every distinct (weapon, magazine, magazine count) combination already
// mounted anywhere on this exact vehicle (excluding the horn) so a curator can pick a real, known-
// working pairing instead of typing a classname from memory - dynamic, vehicle-specific, and always
// accurate for this vehicle's own weapons since it reads them directly off the live object, the same
// "never type what you can read off a real vehicle" principle Waldo_fnc_VehicleWeaponLoadoutCopy uses
// across vehicles. Index 0 is always "Type manually", preserving the existing typed-field workflow.
...
private _pickupLabels = ["Type manually (use the fields below)"];
{
    ...
    if !(_turretIsHornOnly select _forEachIndex) then {
        ...
        private _magazines = (_objectPos magazinesTurret _path) arrayIntersect (_objectPos magazinesTurret _path);
        {
            ...
            private _key = str [_weaponClass, _magForPickup, _magCount];
            if !(_key in _pickupKeys) then {
                _pickupKeys pushBack _key;
                _pickupLabels pushBack format ["%1 + %2 (from Turret %3)", _weaponClass, if (_magForPickup == "") then {"no magazine"} else {_magForPickup}, _path];
            };
        } forEach _weapons;
    };
} forEach _turretPaths;
...
[
    ...
    [
        ...
        ["SLIDER", ["Magazine Count", "Rounds loaded into a turret magazine. Ignored for pylons, for Remove/Clear, and when Copy Weapon From picked something."], [1, 200, 1, 0], false],
    ...
    {
        params ["_args", "_pos"];
        _args params ["_targetIndex", "_turretPathKey", "_turretActionIndex", "_pickupKey", "_weaponClass", "_magazineClass", "_magazineCount", "_pylonIndexKey", "_pylonActionIndex", "_exportOnly"];
        ...
        if (_pickupKey != "MANUAL") then {
            ...
            if (_pickup isEqualType [] && {count _pickup == 3}) then {
                _pickup params ["_pickupWeapon", "_pickupMag", "_pickupCount"];
