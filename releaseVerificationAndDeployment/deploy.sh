...
# The release tag is the single source of truth for the shipped version: rewrite
# description.ext's onLoadName here (in this build's working copy only, not committed)
# so the packaged mission's title and the regenerated cover below always match the tag,
# with no manual pre-bump step required. deploy.yml separately syncs this same version
# and cover image back onto main after a successful build.
