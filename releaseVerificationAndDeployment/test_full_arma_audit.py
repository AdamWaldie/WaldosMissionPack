...
class FullAuditTests(unittest.TestCase):
    ...
    def test_acre_rack_setup_uses_synchronised_server_state_and_never_carried_radio_setup(self):
        ...
        self.assertIn('CBA_fnc_players', setup)
        ...
        self.assertIn('acre_api_fnc_unmountRackRadio', apply)
        self.assertIn('UNMOUNT_RADIO', apply)
        self.assertIn('_resolveNamedNet', apply)
        self.assertIn('RADIO_DATA_NOT_SYNCHRONIZED', apply)
        self.assertIn('FREQUENCY_REQUIRES_PRESET', apply)
        self.assertNotIn('acre_api_fnc_setupRadios', apply)
        self.assertNotIn('remoteExecCall ["Waldo_fnc_ACRE2RackClientAction"', apply)
        self.assertIn('acre_api_fnc_getRadioChannel', apply)
    ...
    def test_acre_rack_profiles_are_beginner_facing_and_validated(self):
        ...
        self.assertIn('"EXISTING_RACKS_CHANNEL_5"', config)
        ...
        self.assertIn('shortName must contain 1-5 characters', validate)
        generator = (ROOT / "releaseVerificationAndDeployment" / "generate_full_arma_audit_mission.py").read_text(encoding="utf-8")
        audit_server = (ROOT / "releaseVerificationAndDeployment" / "fullArmaAudit" / "WMP_FPA.VR" / "featureRangeServer.sqf").read_text(encoding="utf-8")
        audit_client = (ROOT / "releaseVerificationAndDeployment" / "fullArmaAudit" / "WMP_FPA.VR" / "featureRangeClient.sqf").read_text(encoding="utf-8")
