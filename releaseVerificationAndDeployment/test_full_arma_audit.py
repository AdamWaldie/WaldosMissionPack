...
class FullAuditTests(unittest.TestCase):
    ...
    def test_transport_services_have_repeat_safe_blue_identification_and_clear_usage(self):
        ...
        self.assertIn('_playerPassengers isEqualTo [] || {serverTime >= _deadline}', report)
        self.assertIn('private _exitDeadline = serverTime + 10', report)
        self.assertIn('the aircraft does not receive RTB in the same frame passengers are ordered out', report)
        ...
        self.assertIn('private _playerPassengers = (crew _vehicle) select {!(_x in _baseCrew) && {isPlayer _x}}', report)
        self.assertIn('_playerPassengers isEqualTo [] || {serverTime >= _deadline}', report)
        ...
        self.assertIn('if (_destinationLanding) then {"SCRIPTED"} else {"TR UNLOAD"}', dispatch)
        self.assertIn('_waypoint setWaypointScript "A3\\functions_f\\waypoints\\fn_wpLand.sqf"', dispatch)
