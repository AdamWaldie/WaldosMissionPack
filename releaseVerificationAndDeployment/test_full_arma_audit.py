...
class FullAuditTests(unittest.TestCase):
    ...
    def test_new_feature_compositions_are_functional_examples(self):
        ...
        self.assertIn('[""orbit"", ""gunshipOrbit""]', gunship_full)
        self.assertNotIn('getMarkerPos ""gunshipOrbit""', gunship_full)
        self.assertIn('deleteMarker _orbitSourceMarker', gunship_register)
        self.assertIn('remoteExecCall ["Waldo_fnc_HideSetupMarkerLocal", 0,', gunship_register)
    ...
    def test_transport_services_have_repeat_safe_blue_identification_and_clear_usage(self):
        ...
        auto_rtb = (root / "transportAutoRtbServer.sqf").read_text(encoding="utf-8")
        stop = (root / "transportStopGroupLocal.sqf").read_text(encoding="utf-8")
        ...
        runtime_apply = (ROOT / "MissionScripts" / "ZenModules" / "RuntimeControl" / "featureRuntimeApply.sqf").read_text(encoding="utf-8")
        ...
        self.assertIn("class TransportAvailableChildrenLocal", functions)
        self.assertIn("class TransportSetProtectionLocal", functions)
        self.assertIn("class TransportRefreshProtectionServer", functions)
        self.assertIn("class TransportSetGroupNameLocal", functions)
        self.assertNotIn('["Service ID", "Unique setup key', runtime_zen)
        self.assertIn('[_target, _type, "", _name, _options]', runtime_apply)
        ...
        self.assertIn('"Send to Destination"', info)
        self.assertIn('[_vehicle, 1, ["ACE_SelfActions"], _crewRoot]', info)
        self.assertIn('[_vehicle, 1, ["ACE_SelfActions", _crewRootId], _destination]', info)
        self.assertIn('[_vehicle, 1, ["ACE_SelfActions", _crewRootId], _rtb]', info)
        self.assertIn('vehicle _player isEqualTo _target', info)
        self.assertIn('Waldo_TransportService_AceActionVersion', info)
        self.assertIn('_requester in crew _vehicle', request)
        self.assertIn('case "SET_DESTINATION": {_isCurator || {_isCrew}};', request)
        self.assertIn('"AVAILABLE", "TO_PICKUP", "BOARDING", "TO_DESTINATION", "DISEMBARKING", "STUCK"', request)
        self.assertIn('case "RTB": {_isCurator || {_isCrew} || {_ownsRequest}};', request)
        self.assertNotIn("Take Manual Control", info)
        self.assertIn('_vehicle land "LAND"', stop)
        self.assertNotIn('Waldo_TransportService_LocalHoldToken', stop + dispatch)
        self.assertNotIn('keepEngineOnAway', stop + dispatch + report + register)
        self.assertIn('_entry getOrDefault ["state", ""] != "DISEMBARKING"', report)
        self.assertIn('fullCrew [_vehicle, "", false]', report)
        self.assertIn('!isNull _x && {isPlayer _x}', report)
        self.assertIn('Waldo_Transport_DestinationSettleSeconds', report)
        self.assertIn('Waldo_Transport_DestinationEmptyConfirmSeconds', report)
        self.assertIn('Waldo_Transport_DestinationSettleSpeedKph', report)
        self.assertIn('_settled && _empty', report)
        self.assertIn('remoteExecutedOwner in [0, 2]', request)
        self.assertIn('if (!_internalRtb && {remoteExecutedOwner > 0}', request)
        self.assertNotIn('if (remoteExecutedOwner > 0 && {owner _requester != remoteExecutedOwner})', request)
        self.assertIn('remoteExecCall ["Waldo_fnc_TransportAutoRtbServer", 2]', report)
        self.assertIn('Destination settled and human-empty; requesting RTB', report)
        self.assertNotIn('["RTB", _entry get "type", _vehicle, [], _requester] call Waldo_fnc_TransportRequestServer', report)
        self.assertIn('call Waldo_fnc_TransportRequestServer', auto_rtb)
        self.assertIn('Automatic RTB result:', auto_rtb)
        self.assertIn('_entry getOrDefault ["requestId", -1] != _requestId', auto_rtb)
        self.assertIn('_entry getOrDefault ["state", ""] != "DISEMBARKING"', auto_rtb)
        self.assertIn('class TransportAutoRtbServer', functions)
        ...
        self.assertNotIn('call Waldo_fnc_TransportStopGroupLocal;', dispatch)
        hazard_init = (
        ...
        hazard_read = (
            ROOT / "MissionScripts/EnvironmentalSystems/HazardousEnvironments/hazardReadExposureLocal.sqf"
        ).read_text(encoding="utf-8")
        hazard_relay = (
            ROOT / "MissionScripts/EnvironmentalSystems/HazardousEnvironments/hazardNotifyRequesterServer.sqf"
        ).read_text(encoding="utf-8")
        ...
        self.assertIn('call Waldo_fnc_FeatureNotifyLocal', hazard_read)
        self.assertIn('remoteExecCall ["Waldo_fnc_HazardNotifyRequesterServer", 2]', hazard_read)
        self.assertNotIn('remoteExecCall ["Waldo_fnc_FeatureNotifyLocal", owner _requester]', hazard_read)
        self.assertIn('owner _target != _caller', hazard_relay)
        self.assertIn('remoteExecCall ["Waldo_fnc_FeatureNotifyLocal", owner _requester]', hazard_relay)
        self.assertIn('class HazardNotifyRequesterServer', functions)
        ...
        self.assertIn("Waldo_TransportService_Requester", manage)
        ...
        self.assertIn('Waldo_TransportService_Requester", _entry getOrDefault', request)
        self.assertIn('Waldo_TransportService_RequesterUID", "", true', report)
        self.assertIn('Waldo_TransportService_Requester", objNull, true', report)
        self.assertIn('Waldo_TransportService_Requester", objNull, true', register)
        self.assertIn('fullCrew [_vehicle, "", false]', report)
        self.assertIn('isPlayer _x', report)
        self.assertNotIn('private _playerPassengers = (crew _vehicle)', report)
        self.assertNotIn('_remainingPlayers isEqualTo [] || {serverTime >= _deadline}', report)
        self.assertNotIn('if ((crew _vehicle findIf {!(_x in _baseCrew)}) >= 0) exitWith {}', report)
        ...
        self.assertIn("physically overlaps", register)
        self.assertIn("boundingBoxReal", register)
        self.assertIn("Waldo_fnc_TransportSetGroupNameLocal", register)
        group_name = (root / "transportSetGroupNameLocal.sqf").read_text(encoding="utf-8")
        self.assertIn('missionNamespace getVariable ["Waldo_Transport_Services"', group_name)
        self.assertIn('getOrDefault ["name", _displayName]', group_name)
        self.assertIn("Group callsign applied", group_name)
        self.assertIn('["TRANSPORT_RTB", [_target]] call Waldo_fnc_FeatureRuntimeApply', runtime_zen)
        self.assertNotIn('["CHECKBOX", ["Confirm", "Cancel the current task', runtime_zen)
        ...
        self.assertIn('!isTouchingGround _vehicle && {_vehicle distance2D _target <= 300}', dispatch)
        self.assertIn('Awaiting physical takeoff before final approach', dispatch)
        ...
        self.assertIn('Waldo_HeliTransport_DefaultLzSearchRadius", 500', register)
        self.assertIn('Waldo_HeliTransport_DefaultLzClearanceScale", 1.5', register)
        self.assertIn('"landingClearanceScale"', register)
        self.assertIn("boundingBoxReal _vehicle", request)
        self.assertIn("private _clearanceRadius = _clearanceHalfWidth max _clearanceHalfLength", request)
        self.assertIn("private _isExactLzSafe", request)
        self.assertIn("isFlatEmpty", request)
        self.assertIn("if ([_requestedTarget] call _isExactLzSafe)", request)
        self.assertNotIn("private _searchRadii = [0", request)
        ...
        self.assertNotIn('_routeAnchor', dispatch)
        self.assertNotIn('_departureWaypoint', dispatch)
        self.assertIn('_group addWaypoint [_target, 0]', dispatch)
    ...
    def test_acre_343_and_persistence_identity_are_not_cross_contaminated(self):
        ...
        self.assertIn("Respawn radio readiness retry", restore)
        self.assertIn("player canAdd _base", restore)
        self.assertIn("player addItem _base", restore)
        self.assertNotIn("_ID_n] call", restore)
    ...
    def test_generated_mission_runs_current_pack_before_audit_hooks(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn('[] spawn Waldo_fnc_InfoText', generated_player)
    ...
    def test_pack_init_cannot_deadlock_on_dedicated_server(self):
        ...
        self.assertNotIn("Waldo_fnc_InfoText", init)
        self.assertIn("[] spawn Waldo_fnc_InfoText;", init_player)
    ...
    def test_intro_waits_for_preload_finished_without_briefing_or_feature_gates(self):
        ...
        self.assertLess(
            info_text.index('\"WMP_InfoText_FauxLoad\" call BIS_fnc_endLoadingScreen;'),
            info_text.index("_textRevealHandle ="),
        )
        self.assertIn('["Waldo_InfoText_FakeLoadHold", 0]', interface_config)
    ...
    def test_acre2_active_lifecycle_is_safe_and_legacy_is_removed(self):
        ...
        self.assertIn('setVariable ["acre_sys_radio_setup", "[]", true]', acre_init)
    ...
    def test_obituary_groups_deaths_by_player_and_endex_uses_one_rotating_card(self):
        ...
        create_objective = (ROOT / "MissionScripts" / "MissionFlowAndUi" / "createObjective.sqf").read_text(encoding="utf-8")
        set_objective = (ROOT / "MissionScripts" / "MissionFlowAndUi" / "setObjectiveState.sqf").read_text(encoding="utf-8")
        self.assertIn('_ledger pushBack [_taskId, _title, _state]', create_objective)
        self.assertIn('Completed %1 | Failed %2 | Cancelled %3 | Outstanding %4', pages)
        self.assertIn('[%2]</t> %3', pages)
        self.assertNotIn('format ["%1 - %2", _x param [0', pages)
        self.assertIn('count _entry < 3', set_objective)

    def test_acre_rack_setup_uses_synchronised_server_state_and_never_carried_radio_setup(self):
        rack_root = ROOT / "MissionScripts" / "MissionInit" / "ACRE2"
        setup = (rack_root / "acre2RackSetup.sqf").read_text(encoding="utf-8")
        apply = (rack_root / "acre2RackApply.sqf").read_text(encoding="utf-8")
        client_apply = (rack_root / "acre2RackClientApply.sqf").read_text(encoding="utf-8")
        client_result = (rack_root / "acre2RackClientResultServer.sqf").read_text(encoding="utf-8")
        hardware = (rack_root / "acre2RackHardwareServer.sqf").read_text(encoding="utf-8")
        refresh = (rack_root / "acre2SchedulePlayerRefresh.sqf").read_text(encoding="utf-8")
        functions = (ROOT / "MissionScripts" / "WaldosFunctions.sqf").read_text(encoding="utf-8")
        self.assertIn('Waldo_ACRE2_RackDesiredConfig', setup)
        self.assertIn('PlayerConnected', setup)
        self.assertIn('allPlayers - (entities "HeadlessClient_F")', setup)
        self.assertIn('Waldo_ACRE2_RackRunningSignature', setup)
        self.assertIn('rackProfiles', setup)
        self.assertIn('acre_api_fnc_addRackToVehicle', apply)
        self.assertIn('acre_api_fnc_unmountRackRadio', hardware)
        self.assertIn('acre_api_fnc_mountRackRadio', hardware)
        self.assertIn('acre_api_fnc_removeRackFromVehicle', hardware)
        self.assertIn('UNMOUNT_RADIO', client_apply)
        self.assertIn('_resolveNamedNet', client_apply)
        self.assertIn('FREQUENCY_REQUIRES_PRESET', client_apply)
        self.assertNotIn('acre_api_fnc_setupRadios', apply)
        self.assertIn('Waldo_fnc_ACRE2RackClientApply', apply)
        self.assertIn('acre_api_fnc_getRadioChannel', client_apply)
        self.assertNotIn('acre_api_fnc_getMountedRackRadio', apply)
        self.assertIn('Waldo_ACRE2_RackDiagnosticSnapshot', setup)
        self.assertIn('Waldo_ACRE2_RackDiagnosticSnapshot', apply)
        self.assertIn('Waldo_ACRE2_ClientReady', setup)
        self.assertIn('Waldo_ACRE2_ClientReady', apply)
        self.assertIn('ACRE_DATA_SYNCED', refresh)
        self.assertIn('player setVariable ["Waldo_ACRE2_ClientReady", true, true]', refresh)
        self.assertIn('getOrDefault ["readinessTimeoutSeconds", 120]', apply)
        self.assertIn('acre_api_fnc_setVehicleRacksPreset', apply)
        self.assertIn('acre_api_fnc_initVehicleRacks', apply)
        self.assertIn('{(_this select 0) getVariable ["Waldo_ACRE2_ClientReady", false]}', apply)
        self.assertIn('] call acre_api_fnc_addRackToVehicle', apply)
        self.assertIn('acre_api_fnc_setRadioChannel', client_apply)
        self.assertIn('acre_api_fnc_getRadioChannel', client_apply)
        ...
        self.assertIn('Waldo_ACRE2_RackDesiredProfile', setup)
        self.assertIn('ACRE2RackClientApply', functions)
        self.assertIn('ACRE2RackClientResultServer', functions)
        self.assertIn('ACRE2RackHardwareServer', functions)
        self.assertIn('remoteExecutedOwner != 2', client_apply)
        self.assertIn('remoteExecutedOwner <= 2', client_result)
        self.assertIn('remoteExecutedOwner <= 2', hardware)
        self.assertIn('Waldo_ACRE2_RackClientToken', client_result)
    ...
    def test_acre_rack_profiles_are_beginner_facing_and_validated(self):
        ...
        self.assertIn('["netSide", "WEST"]', config)
        self.assertIn('"AIRGND", "ACRE_PRC152"', config)
        self.assertIn('"PLT1", "ACRE_PRC117F"', config)
        ...
        audit_server = (ROOT / "releaseVerificationAndDeployment" / "fullArmaAudit" / "WMP_FPA.VR" / "featureRangeServer.sqf").read_text(encoding="utf-8")
        audit_client = (ROOT / "releaseVerificationAndDeployment" / "fullArmaAudit" / "WMP_FPA.VR" / "featureRangeClient.sqf").read_text(encoding="utf-8")
        self.assertIn('fixture("qa_acre_rack_vehicle", "B_MRAP_01_F"', generator)
    ...
    def test_mhq_and_vvd_integrations_are_repeat_safe_and_complete(self):
        ...
        self.assertIn('!(_x isKindOf "Logic")', mhq)
        self.assertIn("_deployParts arrayIntersect _deployParts", mhq)
        self.assertIn("attachedTo _cursor", mhq)
        self.assertNotIn("[_syncLogic, _target] call BIS_fnc_attachToRelative", mhq)
    ...
    def test_economy_buildables_never_use_visual_fallback_objects(self):
        ...
        self.assertIn('WaldoEcoCore_ZeusHeaderRootText = "WMP Economy Systems";', economy)
        self.assertNotIn('WaldoEcoCore_ZeusHeaderRootText = "Waldos Economy Systems";', economy)
        zen_modules = (ROOT / "MissionScripts" / "EconomySystems" / "Core" / "registerZenModules.sqf").read_text(encoding="utf-8")
        self.assertIn('missionNamespace getVariable ["WaldoEcoCore_ZeusHeaderRootText", "WMP Economy Systems"]', zen_modules)
        self.assertNotIn('private _cat = "Waldos Economy Systems";', zen_modules)
    ...
    def test_safestart_banner_is_compact_and_acknowledgement_is_phase_local(self):
        flow = ROOT / "MissionScripts" / "MissionFlowAndUi"
        hud = (flow / "safeStartHud.sqf").read_text(encoding="utf-8")
        acknowledge = (flow / "safeStartAcknowledgeLocal.sqf").read_text(encoding="utf-8")
        setup = (flow / "setupUiCleanupAction.sqf").read_text(encoding="utf-8")
        apply = (flow / "safeStartApply.sqf").read_text(encoding="utf-8")
        functions = (ROOT / "MissionScripts" / "WaldosFunctions.sqf").read_text(encoding="utf-8")
        self.assertIn("safeZoneW * 0.40", hud)
        self.assertIn('Waldo_SafeStart_AcknowledgedPhase', hud + acknowledge + apply)
    ...
    def test_public_ui_notifications_and_local_cleanup_are_owned_and_repeat_safe(self):
        ...
        zen_send = (ROOT / "MissionScripts" / "ZenModules" / "Zen_notifyModule.sqf").read_text(encoding="utf-8")
        ...
        for function_name in ("ShowUiNotification", "ClearUiPanels", "SetupUiCleanupAction", "SetUiPanelsSuppressed", "SetupUiAcePriority", "SetUiPanelPlacement", "SetLocalUiPanelPlacement", "ReflowUiPanels", "DrainUiNotificationQueue"):
            self.assertIn(f"class {function_name}", functions)
        self.assertIn("class SendNotification", functions)
        self.assertIn("Waldo_fnc_NotificationBroadcast", send)
        self.assertIn('["_recipients", "ALL", ["", west, grpNull, objNull, []]]', send)
        self.assertIn('["audience", "UNITS"]', send)
        self.assertIn('[[], [], _defaultPlayers, 2], false', zen_send)
        self.assertNotIn('[[], [], _defaultPlayers], 2', zen_send)
        paradrop_create = (ROOT / "MissionScripts" / "Paradrop" / "paradropCreateDropZone.sqf").read_text(encoding="utf-8")
        for suffix in ("AREA", "STANDBY", "GREEN", "RED", "POINT"):
            self.assertIn(suffix, paradrop_create)
        self.assertNotIn("_zoneMarker setMarkerAlpha 0", paradrop_create)
        self.assertNotIn("_marker setMarkerAlpha 0", paradrop_create)
        paradrop_quick = (ROOT / "MissionScripts" / "Paradrop" / "paradropQuickFlightSetup.sqf").read_text(encoding="utf-8")
        paradrop_remove = (ROOT / "MissionScripts" / "Paradrop" / "paradropRemoveDropZone.sqf").read_text(encoding="utf-8")
        paradrop_zen = (ROOT / "MissionScripts" / "Paradrop" / "paradropDropZoneZen.sqf").read_text(encoding="utf-8")
        diagnostics_client = (flow / "runDiagnosticsClient.sqf").read_text(encoding="utf-8")
        self.assertIn("Waldo_Paradrop_QuickSetups", paradrop_quick + paradrop_remove)
        self.assertIn('private _point = createMarker [format ["%1_POINT", _prefix], _centre];', paradrop_quick)
        self.assertIn("deleteMarker _target;", paradrop_quick)
        self.assertIn('remoteExecCall ["Waldo_fnc_HideSetupMarkerLocal", 0,', paradrop_quick)
        self.assertNotIn('_target setMarkerType "mil_end";', paradrop_quick)
        self.assertLess(paradrop_quick.index("deleteMarker _target;"), paradrop_quick.index("] spawn {"))
        self.assertIn("published briefing markers", paradrop_quick)
        self.assertIn('["[EDEN] %1 - %2"', paradrop_zen)
        self.assertIn("Waldo_fnc_ParadropRemoveAircraftActionsLocal", paradrop_remove)
        self.assertIn("deleteVehicleCrew _quickAircraft", paradrop_remove)
        self.assertIn("Pre-placed aircraft deletion suppressed because players remain aboard", paradrop_remove)
        self.assertNotIn("Pre-placed Eden aircraft are always retained", paradrop_zen)
        paradrop_create = (ROOT / "MissionScripts" / "Paradrop" / "paradropCreateDropZone.sqf").read_text(encoding="utf-8")
        paradrop_invincibility = (ROOT / "MissionScripts" / "Paradrop" / "paradropSetAircraftInvincibilityLocal.sqf").read_text(encoding="utf-8")
        self.assertIn('"aircraftInvincible",', paradrop_create)
        self.assertIn('"aircraftInvincible",', paradrop_quick)
        ...
        self.assertIn("private _missingJumpCount = 0;", diagnostics_client)
        self.assertIn("} forEach _jumpAircraft;", diagnostics_client)
        self.assertNotIn("} count _jumpAircraft;", diagnostics_client)
        self.assertNotIn("count _missingJump", diagnostics_client)
        hide_setup_marker = (flow / "hideSetupMarkerLocal.sqf").read_text(encoding="utf-8")
        self.assertIn("class HideSetupMarkerLocal", functions)
        self.assertIn("deleteMarkerLocal _markerName", hide_setup_marker)
        self.assertIn("time > 0", hide_setup_marker)
    ...
    def test_optional_zen_modules_and_hc_visual_triage_are_guarded(self):
        ...
        self.assertIn('getVariable ["Waldo_Headless_Enable", false]', toggle)
        self.assertIn('Debug cannot be enabled mid-mission', toggle)
        self.assertIn('Waldo_Headless_DebugGeneration', snapshot)
        self.assertIn('getVariable ["Waldo_Headless_Debug", false]', snapshot)
        self.assertNotIn('while {missionNamespace getVariable ["Waldo_Headless_Enable", false]}', snapshot)
        self.assertIn('getVariable ["Waldo_Headless_Enable", false]', overlay)
        self.assertIn('getVariable ["Waldo_Headless_Debug", false]', overlay)
        ...
        self.assertIn("[46, 48, 49, 51]", diagnostics)
    ...
    def test_dynamic_aa_has_valid_default_detection_and_independent_asset_selection(self):
        ...
        fire_gate = (ROOT / "MissionScripts" / "CombatSystems" / "DynamicAA" / "dynamicAAFireGateLocal.sqf").read_text(encoding="utf-8")
        detector_start = (ROOT / "MissionScripts" / "CombatSystems" / "DynamicAA" / "dynamicAAStartDetectorServer.sqf").read_text(encoding="utf-8")
        functions = (ROOT / "MissionScripts" / "WaldosFunctions.sqf").read_text(encoding="utf-8")
        ...
        self.assertIn('_side createVehicleCrew _vehicle', create)
        self.assertNotIn('createVehicleCrew _vehicle;', create.replace('_side createVehicleCrew _vehicle;', ''))
        self.assertNotIn('(crew _vehicle) joinSilent', create)
        self.assertIn('_side createVehicleCrew _fighter', fighters)
        self.assertNotIn('(crew _fighter) joinSilent', fighters)
        self.assertIn('side _group != _side', create)
        self.assertIn('side _group != _side', fighters)
        self.assertIn('_vehicle setVehicleRadar 1', create)
        self.assertIn('Waldo_DynamicAA_PendingCreation', create)
        self.assertIn('WALDO_INIT_COMPLETE', create)
        self.assertIn('remoteExecCall ["Waldo_fnc_DynamicAAStartDetectorServer", 2]', create)
        self.assertNotIn('spawn Waldo_fnc_DynamicAADetectorLoop', create)
        self.assertIn('remoteExecutedOwner != 2', detector_start)
        self.assertIn('spawn Waldo_fnc_DynamicAADetectorLoop', detector_start)
        self.assertIn('class DynamicAAStartDetectorServer', functions)
        self.assertIn('_group addVehicle _vehicle', create)
        self.assertIn('_group addVehicle _fighter', fighters)
        ...
        self.assertIn('["requestSource", "ZEN"]', placement)
        for setting in ("radius", "engagementRadius", "minimumAltitude", "maximumAltitude", "altitudeMode"):
            self.assertIn(f'["{setting}",', zen)
            self.assertIn(f'getOrDefault ["{setting}"', placement)
        ...
        self.assertIn('netId _x in ["", "0:0"]', create)
        self.assertIn('addCuratorEditableObjects [_editable, false]', create)
        ...
        self.assertIn('_unit disableAI "FIREWEAPON"', group_state)
        self.assertIn('_unit enableAI "FIREWEAPON"', group_state)
        self.assertIn('setVehicleRadar ([0, 1] select _active)', group_state)
        ...
        self.assertIn('class DynamicAAFireGateLocal', functions)
        self.assertIn('Waldo_fnc_DynamicAAFireGateLocal', group_state)
        self.assertIn('addEventHandler ["Fired"', fire_gate)
        self.assertIn('Waldo_DynamicAA_EligibleTargetsLocal', fire_gate)
        self.assertIn('deleteVehicle _projectile', fire_gate)
        self.assertIn('remoteExecutedOwner != 2', fire_gate)
        self.assertIn('including while closed', detector)
        self.assertNotIn('deleteGroupWhenEmpty true', create)
        self.assertNotIn('deleteGroupWhenEmpty true', fighters)
        self.assertIn('private _spawnedCount = 0', fighters)
        self.assertIn('_state set ["fighterWaves"', fighters)
        self.assertIn('private _recipientOwners =', notify)
        self.assertIn('_recipientOwners arrayIntersect _recipientOwners', notify)
        self.assertIn('private _recipientOwners =', fighters)
        self.assertIn('remoteExecCall ["Waldo_fnc_FeatureNotifyLocal", _x]', fighters)
        self.assertIn('[_x, 0] remoteExecCall ["Waldo_fnc_DynamicAASetVehicleAmmo", 2]', destroy)
        self.assertIn('[_x, false, []] remoteExecCall ["Waldo_fnc_DynamicAASetGroupState", 2]', destroy)
        self.assertIn('[_group, false, []] remoteExecCall ["Waldo_fnc_DynamicAASetGroupState", 2]', create)
        self.assertIn('_waypoint setWaypointType "MOVE"', fighters)
        self.assertNotIn('_waypoint setWaypointType "SAD"', fighters)
        self.assertIn('netId _x in ["", "0:0"]', fighters)
        self.assertIn('addCuratorEditableObjects [_editable, false]', fighters)
