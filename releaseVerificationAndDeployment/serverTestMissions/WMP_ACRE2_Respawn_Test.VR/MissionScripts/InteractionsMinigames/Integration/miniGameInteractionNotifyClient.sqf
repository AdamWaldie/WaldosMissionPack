 ...
 * Shows local interaction-equipment acquisition or result feedback without occupying Arma's
 * shared hint channel. The fixed lower-screen notice consumes the active WMP era theme and the
 * player's colour-vision semantic palette; terminal challenge results suppress the duplicate.
 *
 * Arguments:
 * 0: message <STRING> (default "EQUIPMENT UNAVAILABLE")
 * 1: severity <STRING> - WARN, OK or ERROR (default WARN)
 * 2: duration <NUMBER> seconds (default 4)
 *
 * Return Value: BOOL - true when feedback was displayed or intentionally suppressed.
 *
...
if (isNull _frame) then {
    ...
    _frame ctrlSetBackgroundColor (_theme getOrDefault ["panel", [0.025, 0.035, 0.032, 0.95]]);
    _frame ctrlCommit 0;
};
...
private _colour = switch (toUpper _severity) do {
    case "OK": {_theme getOrDefault ["successHex", "#73E2A7"]};
    case "ERROR": {_theme getOrDefault ["dangerHex", "#FF7188"]};
    default {_theme getOrDefault ["warningHex", "#F2BE55"]};
};
...
private _noticeTemplate = "<t align='center' font='" + (_theme getOrDefault ["fontBold", "RobotoCondensedBold"]) + "' size='%1' color='" + _colour + "'>" + _message + "</t>";
