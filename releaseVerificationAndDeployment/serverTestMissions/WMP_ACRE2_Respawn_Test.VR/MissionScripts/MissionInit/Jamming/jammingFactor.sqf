...

{
    ...
    if (!_skip) then {
        ...
        if (!_skip) then {
            ...
            if (_useLos) then {
                // Object/player positions commonly sit exactly on the terrain surface. Passing
                // those raw points to terrainIntersectASL makes the endpoint itself count as an
                // obstruction, so a player standing beside a jammer can incorrectly read 0.
                // Lift only near-ground endpoints; airborne radios retain their true altitude.
                private _losFrom = [_jPos select 0, _jPos select 1, (_jPos select 2) + 2];
                private _losTo = +_pos;
                private _targetAgl = ASLToAGL _losTo;
                if ((_targetAgl select 2) < 1.5) then {
                    _losTo set [2, (_losTo select 2) + 1.5];
                };
                _blocked = terrainIntersectASL [_losFrom, _losTo];
            };
