...
- Airborne Gunship Support - multiple named existing or spawned gunships with side/faction pools, assigned player controllers, validated turret profiles, configurable combat orbits, automatic RTB/service cycles, a FAC-facing Configure Orbit dialog for live radius/altitude adjustment, an off-station status panel, friendly markers (with an orbit-radius footprint ellipse) and focused Zeus operation.
