...
- Vehicle Weapon Loadout - custom turret weapon/ammo add, replace, remove and clear (with real magazine-quantity and per-magazine round-count control, not just one full magazine), plus aircraft pylon set/clear, on any vehicle, script-callable or via a Zeus module placed directly on the target vehicle with its turret/pylon choices discovered live from that vehicle - including a "Copy Weapon From" dynamic picker and clipboard export for a unit's Eden init field.
- Vehicle Appearance - recolor a vehicle's texture slots (a solid colour needs no texture asset at all) and show/hide named model selections, script-callable or via Zeus modules placed directly on the target vehicle; a reusable component catalog lets a curator register a selection name once per vehicle class and pick it by name ever after.
