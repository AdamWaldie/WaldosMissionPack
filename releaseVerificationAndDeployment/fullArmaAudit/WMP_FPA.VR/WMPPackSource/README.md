...
- Smart AI Pass - optional behaviour improvements for non-player AI squads (survivor regroup first), run only on the server and headless clients inside a fixed per-tick budget, with a switch per behaviour and automatic exclusion of player groups and other WMP features' AI. Controlled from `MissionConfig\aiConfig.sqf` or the **AI Control** Zeus module.
