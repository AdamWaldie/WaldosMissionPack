...
private _notify = {params ["_text", ["_state", "INFO"]]; ["FIELD RESUPPLY", _text, _state, "FIELD_RESUPPLY"] remoteExecCall ["Waldo_fnc_FeatureNotifyLocal", owner _unit]};
...
switch (toUpperANSI _operation) do {
    case "REFILL": {
        ...
        if (_hubSide != sideUnknown && {_hubSide getFriend (side group _unit) < 0.6}) exitWith {["This supply hub will not service your side.", "WARNING"] call _notify; false};
        ...
        [format ["Carrier refilled with %1 crate(s).", _issued], "SUCCESS"] call _notify;
        true
    };
    case "DEPLOY": {
    ...
    case "SALVAGE": {
        ...
        private _carried = _unit getVariable ["Waldo_FieldResupply_Crates", 0];
        if (_maximum <= _carried) exitWith {["Carrier capacity is full.", "WARNING"] call _notify; false};
        _unit setVariable ["Waldo_FieldResupply_Crates", _carried + 1, true];
