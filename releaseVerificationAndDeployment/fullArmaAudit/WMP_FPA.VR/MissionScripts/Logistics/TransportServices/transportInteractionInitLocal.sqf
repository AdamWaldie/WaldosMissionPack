...
if (_aceReady) then {
    ...
    private _boat = ["Waldo_Transport_RequestBoat", "Request / Move Pickup", "\a3\ui_f_oldman\data\igui\cfg\holdactions\map_ca.paa", {["REQUEST_PICKUP", "BOAT", objNull] call Waldo_fnc_TransportOpenMapLocal}, {
        private _uid = getPlayerUID player;
        missionNamespace getVariable ["Waldo_BoatTransport_Available", false] || {vehicles findIf {_x getVariable ["Waldo_TransportService_Type", ""] == "BOAT" && {_x getVariable ["Waldo_TransportService_Requester", objNull] isEqualTo player || {_uid != "" && {_x getVariable ["Waldo_TransportService_RequesterUID", ""] == _uid}}}} >= 0}
    }] call ace_interact_menu_fnc_createAction;
    [player, 1, ["ACE_SelfActions", "Waldo_Transport_Root", "Waldo_Transport_BoatRoot"], _boat] call ace_interact_menu_fnc_addActionToObject;
    private _specificBoat = ["Waldo_Transport_SelectBoat", "Select / Manage Transport", "\a3\ui_f\data\igui\cfg\simpletasks\types\documents_ca.paa", {}, {
        private _uid = getPlayerUID player;
        missionNamespace getVariable ["Waldo_BoatTransport_Available", false]
        || {vehicles findIf {_x getVariable ["Waldo_TransportService_Type", ""] == "BOAT" && {_x getVariable ["Waldo_TransportService_Requester", objNull] isEqualTo player || {_uid != "" && {_x getVariable ["Waldo_TransportService_RequesterUID", ""] == _uid}} || {player in crew _x} || {!isNull getAssignedCuratorLogic player}}} >= 0}
    }, {params ["_target", "_player"]; [_player, "BOAT"] call Waldo_fnc_TransportAvailableChildrenLocal}] call ace_interact_menu_fnc_createAction;
    [player, 1, ["ACE_SelfActions", "Waldo_Transport_Root", "Waldo_Transport_BoatRoot"], _specificBoat] call ace_interact_menu_fnc_addActionToObject;
    ...
    player addAction ["<t color='#79C7FF'>Request Boat Transport</t>", {["REQUEST_PICKUP", "BOAT", objNull] call Waldo_fnc_TransportOpenMapLocal}, [], -84, false, true, "", "private _uid = getPlayerUID _this; missionNamespace getVariable ['Waldo_BoatTransport_Available',false] || {vehicles findIf {_x getVariable ['Waldo_TransportService_Type',''] == 'BOAT' && {_x getVariable ['Waldo_TransportService_Requester',objNull] isEqualTo _this || {_uid != '' && {_x getVariable ['Waldo_TransportService_RequesterUID',''] == _uid}}}} >= 0}"];
