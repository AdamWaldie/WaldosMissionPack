...
params[
    "_depotSpawnPoint",
    "_types",
    ["_limitToSideVehicles", false],
    ["_removeUAVs", false],
    ["_lockToken", "", [""]]
];

// Keep direct mission-maker calls compatible by routing them through the same
// server acquisition used by ACE and vanilla actions.
if (hasInterface && {!isNull _depotSpawnPoint} && {_lockToken == ""}) exitWith {
    private _terminal = _depotSpawnPoint getVariable ["Waldo_VVD_Terminal", objNull];
    if (isNull _terminal) then {
        ["Vehicle depot is not fully configured. Run Waldo_fnc_VVDInit before opening it."] call BIS_fnc_guiMessage;
    } else {
        [_terminal, _depotSpawnPoint, _types, _limitToSideVehicles, _removeUAVs, player] call Waldo_fnc_VVDRequestOpenServer;
    };
};
if (!hasInterface || {isNull _depotSpawnPoint} || {_lockToken == ""}) exitWith {
    diag_log format ["[WMP VVD] Local garage launch rejected interface=%1 pad=%2 token=%3", hasInterface, _depotSpawnPoint, _lockToken != ""];
};
...
fnc_Garage_load = {
    ...
    _depotSpawnPointName = _depotSpawnPoint getVariable ["Waldo_VVD_Key", vehicleVarName _depotSpawnPoint];
    if (_depotSpawnPointName == "") then {
        _depotSpawnPointName = ((netId _depotSpawnPoint splitString ": .-") joinString "_");
    };
    ...
    _helipad = createVehicleLocal ["Land_HelipadEmpty_F", ASLToAGL _pos, [], 0, "CAN_COLLIDE"];
    ...
    if (isNull _parentDisplay) exitWith {
        [_depotSpawnPoint, player, _lockToken, "DISPLAY_UNAVAILABLE"] call Waldo_fnc_VVDReleaseOpenServer;
        deleteVehicle _helipad;
        diag_log format ["[WMP VVD] Garage launch failed: gameplay display 46 is unavailable (pad=%1).", netId _depotSpawnPoint];
        ["Vehicle garage could not open because the gameplay display is unavailable."] call BIS_fnc_guiMessage;
    };
    _display = _parentDisplay createDisplay "GarageLoadDisplay";
    if (isNull _display) exitWith {
        [_depotSpawnPoint, player, _lockToken, "DISPLAY_MISSING"] call Waldo_fnc_VVDReleaseOpenServer;
        deleteVehicle _helipad;
        diag_log format ["[WMP VVD] Garage launch failed: GarageLoadDisplay is not defined (pad=%1).", netId _depotSpawnPoint];
        ["Vehicle garage display is missing from description.ext. Include GarageDisplayDefine.hpp."] call BIS_fnc_guiMessage;
    };
    uiNamespace setVariable ["Garage_Display_Loadout", _display];
    diag_log format ["[WMP VVD] Garage opened pad=%1 key=%2 types=%3", netId _depotSpawnPoint, _depotSpawnPointName, _types];
    ...
    // Do not hold the depot forever when the vanilla garage fails to initialise.
    ...
    waitUntil {
        ...
        || {
            !isNull (missionNamespace getVariable ["BIS_fnc_arsenal_center", objNull])
            && {(missionNamespace getVariable ["BIS_fnc_arsenal_center", objNull]) != _helipad}
        }
    };
    if (isNull _display || {diag_tickTime >= _readyDeadline}) exitWith {
        if (!isNull _display) then {_display closeDisplay 1;};
        uiNamespace setVariable ["Garage_Display_Loadout", displayNull];
        deleteVehicle _helipad;
        [_depotSpawnPoint, player, _lockToken, "INITIALISE_FAILED"] call Waldo_fnc_VVDReleaseOpenServer;
        diag_log format ["[WMP VVD] Garage initialisation failed pad=%1", netId _depotSpawnPoint];
        ["Vehicle garage did not initialise. Check the mission RPT for the VVD error."] call BIS_fnc_guiMessage;
    };
    ...
    uiNamespace setVariable ["Garage_Display_Loadout", displayNull];
    [_depotSpawnPoint, player, _lockToken, "CLOSED"] call Waldo_fnc_VVDReleaseOpenServer;
};
...
fnc_Garage_CreateVehicle = {
    ...
    _depotSpawnPointName = _depotSpawnPoint getVariable ["Waldo_VVD_Key", format ["%1", _depotSpawnPoint]];
    _script = missionNamespace getVariable ["Garage_Script_" + _depotSpawnPointName, ""];
    call (compile _script);
};

// The garage lifecycle contains waits while the vanilla lists initialise and
// while the display remains open. Always give it a scheduled context even
// when this public function was reached from an ACE or addAction handler.
[_depotSpawnPoint, _types, _limitToSideVehicles, _removeUAVs, _lockToken] spawn fnc_Garage_load;
