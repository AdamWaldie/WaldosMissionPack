 ...
 * Installs the repeat-safe local packaging action for a registered recovery vehicle. The direct
 * action is omitted when a shared preparation procedure is enabled; that procedure invokes the
 * same server packaging request after success. The server still validates damage, workshop,
 * engineer, occupancy, movement and distance in either route.
 *
 * Arguments:
...
if (_target getVariable ["Waldo_Recovery_InteractionEnabled", false]) exitWith {
