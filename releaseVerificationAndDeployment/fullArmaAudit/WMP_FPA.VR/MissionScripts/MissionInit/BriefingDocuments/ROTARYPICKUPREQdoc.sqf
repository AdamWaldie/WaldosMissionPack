/*
 * Author: WaldoTheWarfighter
 * Adds the rotary-wing pickup request checklist to the player's map diary.
 *
 * Arguments:
 * None
 *
 * Return Value:
 * Nothing
 *
 * Example:
 * call Waldo_fnc_ROTARYPICKUPREQUEST;
 * Locality and authority: Client-local diary record for the current player only.
 * Repeat/JIP: Waldo_fnc_AddDocs guards repeat installation and runs for joining players.
 * Current caller: Waldo_fnc_AddDocs during player-local briefing setup.
 * Result: The player's Checklists diary subject gains the rotary pickup request format.
 */

player createDiaryRecord["Radio Reports",
    ["ROTARY PICKUP REQUEST",
        "
<font color='#addde6'>Misfit 5, this is Misfit 1, PICKUP REQUEST over!</font><br/><br/>
<font color='#addde6'>Line 1: Location</font><br/>
    - 6 or 8 digit grid reference of PZ<br/><br/>
<font color='#addde6'>Line 2: PZ Orientation and type</font><br/>
    - Orientation of approach axis of PZ in degrees and formation to be used by<br/>
       aircraft<br/><br/>
<font color='#addde6'>Line 3: SR Freq and Call-sign</font><br/>
    - SR Freq and Call-sign of requesting unit on ground<br/><br/>
<font color='#addde6'>Line 4: Number of troops</font><br/>
    - Number of troopers to be picked up<br/><br/>
<font color='#addde6'>Line 5: Security at PZ</font><br/>
    - Green - No enemy in area<br/>
    - Yellow/Orange/Amber - Possible enemy in area (include grid locations of<br/>
       known or suspected enemy locations)<br/>
    - Red - Troops in contact at PZ (include grid locations of known or suspected<br/>
       enemy locations)<br/><br/>
<font color='#addde6'>Line 6: Mark Method</font><br/>
    - Smoke<br/>
    - Strobe<br/>
    - Other (Specify)<br/><br/>
        "
    ]
];
