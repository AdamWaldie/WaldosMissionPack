...
private _exitDeadline = diag_tickTime + 0.75;
waitUntil {
    sleep 0.01;
    vehicle _unit isEqualTo _unit || {diag_tickTime >= _exitDeadline}
};
...
if (vehicle _unit isNotEqualTo _unit) exitWith {
    ...
    diag_log format ["[WMP DISMOUNT] Failed to extract local unit %1 from %2.", _unit, _vehicle];
};
if (_vehicle isKindOf "LandVehicle") then {
    private _bounds = boundingBoxReal _vehicle;
    private _halfWidth = (((_bounds select 1) select 0) - ((_bounds select 0) select 0)) * 0.5;
    private _halfLength = (((_bounds select 1) select 1) - ((_bounds select 0) select 1)) * 0.5;
    private _clearRadius = (([_halfWidth, _halfLength] select (_halfLength > _halfWidth)) + 0.8)
        min (["ClearPositionRadius", 6] call _setting);
...
private _targetVelocity = _exitVelocity vectorAdd _throwImpulse;
// The engine can overwrite the first post-moveOut velocity while changing animation/stance.
// Reassert briefly, only while the unit remains out of a vehicle; stop once the launch persists.
for "_attempt" from 0 to 4 do {
    if (vehicle _unit isNotEqualTo _unit) exitWith {};
    _unit setVelocity _targetVelocity;
    sleep 0.025;
    private _observed = velocity _unit;
    if ((_observed vectorDotProduct _throwDirection) >= (_targetVelocity vectorDotProduct _throwDirection) * 0.7
        && {(_observed select 2) >= ((_targetVelocity select 2) * 0.5)}) exitWith {};
};
diag_log format ["[WMP DISMOUNT] class=%1 flip=%2 angular=%3 targetVelocity=%4 observedVelocity=%5",
    typeOf _vehicle, _throwDirection, _flipAngularSpeed, _targetVelocity, velocity _unit];
