...
[] spawn {
    ...
    while {true} do {
        ...
        if (_factor > 0 && {!isNull _uav}) then {
            ...
            if (_notify) then { [_factor, "UAV LINK DEGRADED", 5311, "DATALINK QUALITY REDUCED"] call Waldo_fnc_JammingHud; };
            ...
            if (_factor >= 0.95 && {!_cutThisLink}) then {
                ...
                if (_notify) then {
                    ["UAV LINK LOST", "Move clear of the interference field to restore the datalink.", 6, "WARNING"] call Waldo_fnc_JammingNotice;
                };
            };
        } else {
            ...
            [0, "UAV LINK DEGRADED", 5311] call Waldo_fnc_JammingHud;
        };
