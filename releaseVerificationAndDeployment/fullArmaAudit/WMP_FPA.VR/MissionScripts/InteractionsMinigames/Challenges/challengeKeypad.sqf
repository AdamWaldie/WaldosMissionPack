...
private _display = [
    _title,
    "Reconstruct the access code from the authorized digit bank and adjacent relay checksums.",
    _timeLimit,
    _resolve,
    0.50,
    "Mouse or number keys; Backspace removes; Enter submits",
    "Each relay checksum is the total of two neighbouring slots. All keys remain available; submitted attempts report exact and misplaced digits."
...
private _code = [];
private _pairSums = [];
private _uniqueEvidence = false;
for "_attempt" from 0 to 99 do {
    private _available = +_pool;
    _code = [];
    for "_index" from 1 to _digits do {
        private _pick = floor random count _available;
        _code pushBack (_available deleteAt _pick);
    };
    _pairSums = [];
    for "_index" from 0 to (_digits - 2) do {
        _pairSums pushBack ((_code select _index) + (_code select (_index + 1)));
    };
    private _bank = +_code;
    _bank sort true;
    private _candidateCount = 0;
    {
        ...
        for "_index" from 0 to ((count _pairSums) - 1) do {
            _candidate pushBack ((_pairSums select _index) - (_candidate select _index));
        };
        private _candidateBank = +_candidate;
        _candidateBank sort true;
        if (_candidateBank isEqualTo _bank) then {_candidateCount = _candidateCount + 1;};
    } forEach _bank;
    if (_candidateCount == 1) exitWith {_uniqueEvidence = true;};
};
if (!_uniqueEvidence) then {
    // Deterministic fallback remains solvable by the same evidence contract.
    _code = _pool select [0, _digits];
    _pairSums = [];
    for "_index" from 0 to (_digits - 2) do {
        _pairSums pushBack ((_code select _index) + (_code select (_index + 1)));
    };
...
private _relayLabels = [];
{
    _relayLabels pushBack format ["R%1=%2", _forEachIndex + 1, _x];
} forEach _pairSums;
private _evidenceTemplate = "<t size='%1'><t color='#9FDDB0'>AUTHORIZED BANK:</t> <t color='#FFFFFF'>"
    + (_digitBank joinString "  ")
    + "</t><br/><t color='#9FDDB0'>ADJACENT RELAYS:</t> <t color='#FFFFFF'>"
    + (_relayLabels joinString "  ")
    + "</t><br/><t color='#F2BE55'>R1=SLOT 1+2, R2=SLOT 2+3, AND SO ON</t></t>";
[_evidence, _evidenceTemplate, 1, 0.58] call Waldo_fnc_MiniGameEquipmentFitStructuredText;
...
_display setVariable ["Waldo_MG_KP_Refresh", {
    ...
    if (!isNull _enter) then {_enter ctrlEnable (count _input == _digits); _enter ctrlSetTooltip (if (count _input == _digits) then {"Submit the entered code"} else {format ["Disabled: enter %1 more digit(s)", _digits - count _input]});};
    {
        ...
        if (_value >= 0) then {
            private _available = count _input < _digits;
            _x ctrlEnable _available;
            _x ctrlSetTooltip (if (_available) then {format ["Enter digit %1", _value]} else {"Entry is full; use BACK or ENTER"});
        };
...
_display setVariable ["Waldo_MG_KP_Action", {
    ...
    if (_value >= 0) exitWith {
        if (count _input < _digits) then {_input pushBack _value; _display setVariable ["Waldo_MG_KP_Input", _input]; [_display] call (_display getVariable ["Waldo_MG_KP_Refresh", {}]);};
    };
