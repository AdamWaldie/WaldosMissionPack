...
diag_log format ["[WMP INTERACTION] %1 object=%2 challenge=%3 actor=%4 outcome=%5 reason=%6 attempt=%7 objectLocal=%8 objectOwner=%9 actorOwner=%10 remoteOwner=%11", _state, netId _object, _challengeId, if (isNull _actor) then {"<none>"} else {name _actor}, _outcomeCode, _reason, _attemptId, local _object, owner _object, if (isNull _actor) then {-1} else {owner _actor}, _caller];
if (!isNull _actor && {isPlayer _actor}) then {
    [
        format ["%1: %2", if (_success) then {"PROCEDURE SUCCESS"} else {"PROCEDURE FAILURE"}, _reason],
        if (_success) then {"OK"} else {"ERROR"},
        6
    ] remoteExecCall ["Waldo_fnc_MiniGameInteractionNotifyClient", owner _actor];
};
