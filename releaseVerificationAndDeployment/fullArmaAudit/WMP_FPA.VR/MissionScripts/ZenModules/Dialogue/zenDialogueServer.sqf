 ...
 * Current callers: five dialogue/conversation ZEN handlers. Example: ZEN remote execution only.
 */
params [["_operation", "", [""]], ["_target", objNull, [objNull]], ["_values", [], [[]]], ["_requester", objNull, [objNull]]];
if (!isServer || {isNull _target} || {!(_target isKindOf "CAManBase")} || {isNull _requester}) exitWith {false};
...
switch (toUpperANSI _operation) do {
    ...
    case "ADVANCED_CONTROL": {
        private _control = _values param [0, "START"];
        _ok = if (_control == "CANCEL") then {[_target, _requester, "", "CURATOR_CANCELLED"] call Waldo_fnc_ConversationCancel} else {[_target, _requester] call Waldo_fnc_ConversationStart};
    };
};
["DIALOGUE", if (_ok) then {"Dialogue operation completed."} else {"Dialogue operation was rejected; check the target and registered IDs."}, if (_ok) then {"SUCCESS"} else {"WARNING"}, "DIALOGUE_ZEN", 6] remoteExecCall ["Waldo_fnc_FeatureNotifyLocal", owner _requester];
_ok
