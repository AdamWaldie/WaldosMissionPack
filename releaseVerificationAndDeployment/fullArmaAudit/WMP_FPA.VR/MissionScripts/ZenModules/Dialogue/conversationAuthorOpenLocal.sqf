...
private _draftCombo = _display ctrlCreate ["RscCombo", -1]; _draftCombo ctrlSetPosition [0.05,0.225,0.28,0.032]; _draftCombo ctrlSetTooltip "Switch between conversations saved during this mission."; _draftCombo ctrlCommit 0;
_display setVariable ["WaldoConvAuthor_DraftCombo", _draftCombo];
private _draftNew = [[0.34,0.225,0.09,0.032], "NEW"] call _makeButton; _draftNew ctrlSetTooltip "Start another conversation with an example branch.";
private _draftDuplicate = [[0.435,0.225,0.105,0.032], "COPY"] call _makeButton; _draftDuplicate ctrlSetTooltip "Copy this whole conversation.";
private _draftDelete = [[0.545,0.225,0.09,0.032], "REMOVE"] call _makeButton; _draftDelete ctrlSetTooltip "Remove this saved conversation. At least one must remain.";
...
private _nodeList = _display ctrlCreate ["RscListbox", -1]; _nodeList ctrlSetPosition [0.055,0.302,0.255,0.24]; _nodeList ctrlSetTooltip "A part is one moment in the conversation. Answers can jump to another part."; _nodeList ctrlCommit 0;
...
private _nodeButtons = [];
{
    ...
    [[0.055,0.548,0.042,0.031], "ADD", "Add a new conversation part."],
    [[0.100,0.548,0.049,0.031], "COPY", "Copy the selected part."],
    [[0.152,0.548,0.060,0.031], "REMOVE", "Remove the selected part. At least one must remain."],
    [[0.215,0.548,0.041,0.031], "UP", "Move the selected part earlier."],
    [[0.259,0.548,0.051,0.031], "DOWN", "Move the selected part later."]
];
_display setVariable ["WaldoConvAuthor_NodeButtons", _nodeButtons];
["RscText", [0.055,0.585,0.09,0.023], "EDIT PART NAME"] call _makeText;
private _nodeId = _display ctrlCreate ["RscEdit", -1]; _nodeId ctrlSetPosition [0.145,0.58,0.165,0.032]; _nodeId ctrlSetTooltip "Type a new part name, then click elsewhere. Every route to the old name is updated automatically."; _nodeId ctrlCommit 0;
_display setVariable ["WaldoConvAuthor_NodeId", _nodeId];
["RscText", [0.055,0.622,0.09,0.023], "IF NO ANSWERS"] call _makeText;
private _autoNext = _display ctrlCreate ["RscCombo", -1]; _autoNext ctrlSetPosition [0.145,0.617,0.165,0.032]; _autoNext ctrlSetTooltip "Used only when this part has no player answers. Choose END to finish."; _autoNext ctrlCommit 0;
_display setVariable ["WaldoConvAuthor_AutoNext", _autoNext];
private _routeHelp = ["RscStructuredText", [0.055,0.655,0.255,0.055], "<t size='0.64'>Select a part to see where its answers lead.</t>"] call _makeText;
_display setVariable ["WaldoConvAuthor_RouteHelp", _routeHelp];

["RscText", [0.34,0.275,0.285,0.025], "2  WHAT THE NPC SAYS"] call _makeText;
private _lineList = _display ctrlCreate ["RscListbox", -1]; _lineList ctrlSetPosition [0.34,0.302,0.29,0.14]; _lineList ctrlSetTooltip "Select a line to edit it below."; _lineList ctrlCommit 0;
...
private _lineButtons = [];
{
    ...
    [[0.340,0.448,0.045,0.031], "ADD", "Add another NPC line to this part."],
    [[0.389,0.448,0.052,0.031], "COPY", "Copy the selected NPC line."],
    [[0.445,0.448,0.065,0.031], "REMOVE", "Remove the selected NPC line."],
    [[0.514,0.448,0.045,0.031], "UP", "Move the selected line earlier."],
    [[0.563,0.448,0.067,0.031], "DOWN", "Move the selected line later."]
];
_display setVariable ["WaldoConvAuthor_LineButtons", _lineButtons];
["RscText", [0.34,0.486,0.12,0.023], "NPC DIALOGUE"] call _makeText;
private _lineText = _display ctrlCreate ["RscEditMulti", -1]; _lineText ctrlSetPosition [0.34,0.51,0.29,0.074]; _lineText ctrlSetTooltip "The subtitle spoken by the NPC."; _lineText ctrlCommit 0;
_display setVariable ["WaldoConvAuthor_LineText", _lineText];
["RscText", [0.34,0.59,0.08,0.023], "SOUND"] call _makeText;
private _soundCombo = _display ctrlCreate ["RscCombo", -1]; _soundCombo ctrlSetPosition [0.42,0.586,0.21,0.032]; _soundCombo ctrlSetTooltip "Optional mission sound. Leave as None for subtitles only."; _soundCombo ctrlCommit 0;
...
["RscText", [0.34,0.625,0.12,0.023], "SOUND LENGTH"] call _makeText;
private _soundDuration = _display ctrlCreate ["RscEdit", -1]; _soundDuration ctrlSetPosition [0.46,0.621,0.055,0.031]; _soundDuration ctrlSetText "-1"; _soundDuration ctrlSetTooltip "Sound length in seconds. Leave -1 to use automatic timing."; _soundDuration ctrlCommit 0;
_display setVariable ["WaldoConvAuthor_SoundDuration", _soundDuration];
["RscText", [0.52,0.625,0.07,0.023], "TEXT TIME"] call _makeText;
private _textDuration = _display ctrlCreate ["RscEdit", -1]; _textDuration ctrlSetPosition [0.575,0.621,0.055,0.031]; _textDuration ctrlSetText "-1"; _textDuration ctrlSetTooltip "Subtitle time in seconds. Leave -1 to calculate it from the text."; _textDuration ctrlCommit 0;
_display setVariable ["WaldoConvAuthor_TextDuration", _textDuration];
["RscText", [0.34,0.66,0.08,0.023], "GESTURE"] call _makeText;
private _gestureCombo = _display ctrlCreate ["RscCombo", -1]; _gestureCombo ctrlSetPosition [0.42,0.656,0.21,0.032]; _gestureCombo ctrlSetTooltip "Optional gesture played while this line is spoken."; _gestureCombo ctrlCommit 0;
...
private _choiceList = _display ctrlCreate ["RscListbox", -1]; _choiceList ctrlSetPosition [0.66,0.302,0.285,0.14]; _choiceList ctrlSetTooltip "Select a player answer to edit it below."; _choiceList ctrlCommit 0;
...
private _choiceButtons = [];
{
    ...
    [[0.660,0.448,0.045,0.031], "ADD", "Add a player answer to this part."],
    [[0.709,0.448,0.052,0.031], "COPY", "Copy the selected player answer."],
    [[0.765,0.448,0.064,0.031], "REMOVE", "Remove the selected player answer."],
    [[0.833,0.448,0.043,0.031], "UP", "Move the selected answer earlier."],
    [[0.880,0.448,0.065,0.031], "DOWN", "Move the selected answer later."]
];
_display setVariable ["WaldoConvAuthor_ChoiceButtons", _choiceButtons];
["RscText", [0.66,0.486,0.10,0.023], "PLAYER ANSWER"] call _makeText;
private _choiceLabel = _display ctrlCreate ["RscEditMulti", -1]; _choiceLabel ctrlSetPosition [0.66,0.51,0.285,0.074]; _choiceLabel ctrlSetTooltip "The answer shown to the player."; _choiceLabel ctrlCommit 0;
_display setVariable ["WaldoConvAuthor_ChoiceLabel", _choiceLabel];
["RscText", [0.66,0.59,0.10,0.023], "ANSWER NAME"] call _makeText;
private _choiceId = _display ctrlCreate ["RscEdit", -1]; _choiceId ctrlSetPosition [0.755,0.586,0.19,0.032]; _choiceId ctrlSetTooltip "A stable internal name for this answer. The generated name is safe to keep."; _choiceId ctrlCommit 0;
_display setVariable ["WaldoConvAuthor_ChoiceId", _choiceId];
["RscText", [0.66,0.627,0.10,0.023], "THEN GO TO"] call _makeText;
private _choiceDestination = _display ctrlCreate ["RscCombo", -1]; _choiceDestination ctrlSetPosition [0.755,0.623,0.19,0.032]; _choiceDestination ctrlSetTooltip "Choose the next conversation part, or END to finish."; _choiceDestination ctrlCommit 0;
_display setVariable ["WaldoConvAuthor_ChoiceDestination", _choiceDestination];
["RscStructuredText", [0.66,0.66,0.285,0.05], "<t size='0.64'>Choose another part to continue, or END to finish the conversation.</t>"] call _makeText;
...
private _replaceCheck = _display ctrlCreate ["RscCheckBox", -1]; _replaceCheck ctrlSetPosition [0.05,0.802,0.026,0.026]; _replaceCheck cbSetChecked false; _replaceCheck ctrlCommit 0;
_display setVariable ["WaldoConvAuthor_Replace", _replaceCheck];
["RscText", [0.078,0.802,0.14,0.026], "REPLACE SAME NAME"] call _makeText;
_replaceCheck ctrlSetTooltip "Required only when a conversation with this name is already saved on the server.";
private _removeCheck = _display ctrlCreate ["RscCheckBox", -1]; _removeCheck ctrlSetPosition [0.225,0.802,0.026,0.026]; _removeCheck cbSetChecked false; _removeCheck ctrlSetTooltip "Remove the assignment after the NPC completes it once."; _removeCheck ctrlCommit 0;
_display setVariable ["WaldoConvAuthor_RemoveAfter", _removeCheck];
["RscText", [0.253,0.802,0.10,0.026], "ONE USE ONLY"] call _makeText;
...
private _register = [[0.465,0.798,0.12,0.033], "SAVE FOR ZEUS"] call _makeButton; _register ctrlSetTooltip "Make this conversation available in Conversation: Assign.";
private _assign = [[0.59,0.798,0.15,0.033], "SAVE + GIVE TO NPC"] call _makeButton; _assign ctrlSetTooltip "Save this conversation and give it to the selected NPC now.";
private _assignGroup = [[0.745,0.798,0.17,0.033], "SAVE + GIVE TO GROUP"] call _makeButton; _assignGroup ctrlSetTooltip "Save this conversation and give it to every NPC in the selected NPC's group.";
_assign ctrlEnable (!isNull _target && {alive _target}); _assignGroup ctrlEnable (!isNull _target && {alive _target});
private _exportConfig = [[0.05,0.84,0.18,0.035], "COPY FOR MISSION CONFIG"] call _makeButton; _exportConfig ctrlSetTooltip "Copy a safe row for MissionConfig dialogueConfig.sqf.";
private _exportScript = [[0.235,0.84,0.18,0.035], "COPY FOR SERVER SCRIPT"] call _makeButton; _exportScript ctrlSetTooltip "Copy an advanced ConversationCreate example for a server-side mission script.";
