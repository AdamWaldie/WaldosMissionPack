...
class PrReviewAuditTests(unittest.TestCase):
    ...
    def test_accessibility_audit_uses_current_tester_without_changing_pack_default(self):
        ...
        self.assertIn("private _headPosition = ASLToAGL eyePos _unit;", pid)
        self.assertNotIn('selectionPosition "head"', pid)
    ...
    def test_selected_features_are_not_registered_as_zen_modules(self):
        ...
        self.assertIn('Waldo_ZenModuleCount", 48', source)
    ...
    def test_audit_zeus_follows_the_replacement_player_unit(self):
        ...
        self.assertIn('_object isKindOf "CAManBase"', server)
        self.assertIn('netId _networkObject != "0:0"', server)
        self.assertIn('objectFromNetId (netId _networkObject)', server)
        self.assertIn('missionNamespace setVariable ["Waldo_QA_FeatureObjects", _objects];', server)
        self.assertNotIn('missionNamespace setVariable ["Waldo_QA_FeatureObjects", _objects, true]', server)
        self.assertNotIn('missionNamespace setVariable ["Waldo_QA_AARTarget", _target, true]', server)
    ...
    def test_dynamic_paradrop_is_authoritative_configurable_and_side_independent(self):
        ...
        route = (root / "paradropBuildFlightRoute.sqf").read_text(encoding="utf-8")
        quick = (root / "paradropQuickFlightSetup.sqf").read_text(encoding="utf-8")
        normalize = (root / "paradropNormalizeJumpEnvelope.sqf").read_text(encoding="utf-8")
        ...
        configure_networked = (root / "paradropConfigureAircraftNetworkedLocal.sqf").read_text(encoding="utf-8")
        ...
        self.assertIn('setBehaviourStrong "CARELESS"', route)
        self.assertIn('setCombatMode "BLUE"', route)
        # Match ZEN's proven Fly Height operation. Passenger Blackfish dynamically spawned at the
        # requested altitude regressed to its ~100 m default with the forced-array alternative.
        self.assertIn("flyInHeight _altitude", route)
        ...
        self.assertIn('"Jump methods"', zen)
        self.assertIn('[["STATIC", "HALO", "BOTH"]', zen)
        self.assertIn('"Route altitude"', zen)
        self.assertIn('"Route speed"', zen)
        self.assertNotIn('"Use custom route profile"', zen)
        self.assertNotIn('"customRoute"', zen)
        self.assertIn('Waldo_Paradrop_DefaultStaticRouteAltitude', zen)
        self.assertIn('Waldo_Paradrop_DefaultStaticRouteSpeed', zen)
        self.assertIn('missionNamespace getVariable ["WALDO_STATIC_MINALTITUDE", 180]', zen)
        self.assertIn('missionNamespace getVariable ["WALDO_STATIC_MAXSPEED", 310]', zen)
        self.assertIn('private _jumpMethods = toUpperANSI', create)
        self.assertIn('private _configuredHaloMinimum = missionNamespace getVariable ["WALDO_PARA_HALOALTITUDE", 1000]', create)
        ...
        self.assertIn('Waldo_Paradrop_DefaultStaticRouteSpeed', quick)
        self.assertIn('if (_staticEnabled) then {_maxSpeed = _maxSpeed min _staticSpeedCeiling}', quick)
        self.assertIn('["_altitude", 300, [0]]', normalize)
        self.assertIn('["staticMaximumSpeed", (_rawStaticMaxSpeed max 0) min 700]', normalize)
        self.assertNotIn('_maxSpeed + _staticMaxSpeedBuffer', normalize)
        self.assertIn('["staticMinimumAltitude", _staticMinimum]', zen)
        self.assertIn('["haloMinimumAltitude", _haloMinimum]', zen)
        self.assertNotIn('"Enable static-line jump"', zen)
        self.assertNotIn('"Enable HALO jump"', zen)
        self.assertNotIn('"Require open ramp/door"', zen)
        self.assertIn('["requireOpenDoor", false]', zen)
        ...
        self.assertIn("private _condition", static_jump)
        self.assertIn('_condition,\n    "true",', static_jump)
        self.assertNotIn("_progressCondition", static_jump)
        self.assertIn("private _condition", halo_jump)
        self.assertIn('_condition,\n    "true",', halo_jump)
        self.assertNotIn("_progressCondition", halo_jump)
        self.assertIn('"PARADROP STATUS"', jump_status)
        self.assertIn('call Waldo_fnc_FeatureNotifyLocal', jump_status)
        ...
        self.assertIn('_unit disableAI "AUTOTARGET"', dynamic_aa_state)
        self.assertNotIn('_unit enableAI "AUTOTARGET"', dynamic_aa_state)
        self.assertIn('_eligibleTargets', dynamic_aa_state)
        self.assertIn('_group ignoreTarget _target', dynamic_aa_state)
        self.assertIn('_group forgetTarget _target', dynamic_aa_state)
        self.assertIn('setVehicleReceiveRemoteTargets false', dynamic_aa_state)
        self.assertIn('_unit disableAI "FIREWEAPON"', dynamic_aa_state)
        self.assertIn('_unit enableAI "FIREWEAPON"', dynamic_aa_state)
        ...
        self.assertIn('[_x, 0] remoteExecCall ["Waldo_fnc_DynamicAASetVehicleAmmo", 2]', dynamic_aa_destroy)
        ...
        paradrop_invincibility = (scripts / "Paradrop" / "paradropSetAircraftInvincibilityLocal.sqf").read_text(encoding="utf-8")
        paradrop_quick = (scripts / "Paradrop" / "paradropQuickFlightSetup.sqf").read_text(encoding="utf-8")
        paradrop_zen = (scripts / "Paradrop" / "paradropDropZoneZen.sqf").read_text(encoding="utf-8")
        self.assertIn('"aircraftInvincible",', create)
        self.assertIn("Waldo_Paradrop_DefaultAircraftInvincible", create)
    ...
    def test_dynamic_ao_runtime_generator_and_recent_regressions_are_wired(self):
        ...
        envelope = (ROOT / "MissionScripts" / "Paradrop" / "paradropNormalizeJumpEnvelope.sqf").read_text(encoding="utf-8")
        embark = (ROOT / "MissionScripts" / "Paradrop" / "paradropEmbarkLocal.sqf").read_text(encoding="utf-8")
        self.assertIn('call Waldo_fnc_ParadropNormalizeJumpEnvelope', paradrop)
        self.assertIn('["staticMaximumSpeed", (_rawStaticMaxSpeed max 0) min 700]', envelope)
        self.assertNotIn('_maxSpeed + _staticMaxSpeedBuffer', envelope)
        self.assertIn('_altitude + _staticMaxAltitudeBuffer', envelope)
